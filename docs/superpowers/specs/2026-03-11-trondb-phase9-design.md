...
node_id = "node-2"
role = "replica"
addr = "192.0.2.11:9400"

[[cluster.peers]]
node_id = "router-1"
role = "router"
addr = "192.0.2.12:9400"
```
...
- `TRONDB_PEERS` — comma-separated `node_id:role:addr` entries (e.g. `node-2:replica:192.0.2.11:9400,router-1:router:192.0.2.12:9400`). Note: the `addr` component contains a colon (host:port), so parsing splits on comma first, then splits each entry into exactly 3 parts: `node_id`, `role`, and the remainder as `addr`
