...
impl Parser {
            ...
            Some(Token::Alter) => {
                ...
                match self.peek() {
                    Some(Token::Entity) => {
                        ...
                        Ok(Statement::AlterEntityDropAffinity(
                            AlterEntityDropAffinityStmt { entity_id },
                        ))
                    }
                        ...
                        match self.peek() {
                            Some(Token::Rename) => {
                                ...
                                Ok(Statement::AlterCollection(AlterCollectionStmt {
                                    collection,
                                    operation: AlterCollectionOp::RenameField {
                                        old_name,
                                        new_name,
                                    },
                                }))
                                ...
                                match self.peek() {
                                    Some(Token::Fields) => {
                                        ...
                                        Ok(Statement::AlterCollection(AlterCollectionStmt {
                                            collection,
                                            operation:
                                                AlterCollectionOp::AlterRepresentationSetFields {
                                                    repr_name,
                                                    fields,
                                                },
                                        }))
                            ...
                            Some(Token::Add) => {
                                ...
                                let metric = match self.peek() {
                                    ...
                                    Some(Token::InnerProduct) => {
                                        self.advance();
                                        Metric::InnerProduct
                                    }
                                    _ => {
                                        return Err(ParseError::InvalidSyntax(
                                            "expected COSINE or INNER_PRODUCT".into(),
                                        ))
                                    }
                                };
                                ...
                                let sparse = if self.peek() == Some(&Token::Sparse) {
                                    ...
                                    match self.peek() {
                                        ...
                                        _ => {
                                            return Err(ParseError::InvalidSyntax(
                                                "expected true or false after SPARSE".into(),
                                            ))
                                        }
                                    }
                                } else {
    ...
    fn parse_create(&mut self) -> Result<Statement, ParseError> {
        ...
        match self.peek() {
            ...
            Some(Token::Affinity) => {
                ...
                self.expect(&Token::Semicolon)?;
                Ok(Statement::CreateAffinityGroup(CreateAffinityGroupStmt {
                    name,
                }))
            }
            Some(tok) => {
    ...
    fn parse_create_collection(&mut self) -> Result<Statement, ParseError> {
        ...
        let vectoriser_config = if vec_model.is_some()
            || vec_model_path.is_some()
            || vec_device.is_some()
            || vec_type.is_some()
            || vec_endpoint.is_some()
            || vec_auth.is_some()
        {
    ...
    fn try_parse_qualified_field(&mut self) -> Result<Option<QualifiedField>, ParseError> {
        ...
        if let Some(Token::Ident(_)) = self.peek() {
            ...
            if self.pos + 1 < self.tokens.len() && self.tokens[self.pos + 1].0 == Token::Dot {
        ...
        loop {
            ...
            let on_left = self.try_parse_qualified_field()?.ok_or_else(|| {
                ParseError::InvalidQuery(
                    "expected qualified field in ON clause (alias.field)".into(),
                )
            })?;
            self.expect(&Token::Eq)?;
    ...
    fn parse_where_comparison(&mut self) -> Result<WhereClause, ParseError> {
        ...
        let op = self
            .advance()
            .ok_or_else(|| ParseError::UnexpectedEof("expected operator".into()))?;
...
mod tests {
    ...
    fn parse_search_hybrid() {
        let stmt =
            parse("SEARCH venues NEAR VECTOR [0.1, 0.2] NEAR SPARSE [1:0.8] LIMIT 10;").unwrap();
        match stmt {
            Statement::Search(s) => {
                assert!(s.dense_vector.is_some());
    ...
    fn parse_search_with_where() {
        let stmt = parse("SEARCH venues WHERE h3_res4 = '89283' NEAR VECTOR [0.1, 0.2] LIMIT 10;")
            .unwrap();
        match stmt {
            Statement::Search(s) => {
    ...
    fn parse_search_near_text_with_where() {
        let stmt =
            parse("SEARCH venues WHERE city = 'Bristol' NEAR 'live jazz' LIMIT 10;").unwrap();
        match stmt {
            Statement::Search(s) => {
                assert!(s.filter.is_some());
    ...
    fn parse_insert_with_named_repr() {
        let stmt = parse(
            "INSERT INTO venues (id, name) VALUES ('v1', 'Pub')
    ...
    fn parse_insert_multiple_reprs() {
        let stmt = parse(
            ...
            REPRESENTATION sparse SPARSE [1:0.5];",
        )
        .unwrap();
        match stmt {
    ...
    fn parse_fetch_where_and() {
        let stmt =
            parse("FETCH * FROM venues WHERE city = 'London' AND status = 'active';").unwrap();
        match &stmt {
    ...
    fn parse_fetch_gte_filter() {
        ...
        match stmt {
            Statement::Fetch(f) => {
                assert_eq!(
                    f.filter,
                    Some(WhereClause::Gte("score".into(), Literal::Int(80)))
                );
            }
            _ => panic!("expected Fetch"),
        }
    ...
    fn parse_fetch_lte_filter() {
        ...
        match stmt {
            Statement::Fetch(f) => {
                assert_eq!(
                    f.filter,
                    Some(WhereClause::Lte("score".into(), Literal::Int(20)))
                );
            }
            _ => panic!("expected Fetch"),
        }
    ...
    fn parse_create_edge_decay_linear() {
        let stmt =
            parse("CREATE EDGE knows FROM people TO people DECAY LINEAR RATE 0.01 FLOOR 0.2;")
                .unwrap();
        match stmt {
            Statement::CreateEdgeType(s) => {
                let dc = s.decay_config.unwrap();
    ...
    fn parse_create_edge_decay_step() {
        let stmt = parse("CREATE EDGE knows FROM people TO people DECAY STEP RATE 3600;").unwrap();
        match stmt {
            Statement::CreateEdgeType(s) => {
                let dc = s.decay_config.unwrap();
    ...
    fn parse_update_multiple_fields() {
        let stmt =
            parse("UPDATE 'v1' IN venues SET name = 'X', score = 42, active = true;").unwrap();
        match stmt {
            Statement::Update(u) => {
                assert_eq!(u.assignments.len(), 3);
                assert_eq!(
                    u.assignments[0],
                    ("name".into(), Literal::String("X".into()))
                );
    ...
    fn parse_confirm_edge() {
        let stmt =
            parse("CONFIRM EDGE FROM 'e1' TO 'e2' TYPE performs_at CONFIDENCE 0.95;").unwrap();
        match stmt {
    ...
    fn parse_where_in_list() {
        let stmt =
            parse("FETCH * FROM venues WHERE category IN ('music', 'theatre', 'comedy');").unwrap();
        match stmt {
            Statement::Fetch(f) => match f.filter {
    ...
    fn parse_where_like() {
        ...
        match stmt {
            Statement::Fetch(f) => {
                assert_eq!(
                    f.filter,
                    Some(WhereClause::Like("name".into(), "Jazz%".into()))
                );
            }
            _ => panic!("expected Fetch"),
        }
    ...
    fn parse_fetch_where_order_by() {
        let stmt =
            parse("FETCH * FROM venues WHERE city = 'London' ORDER BY name LIMIT 5;").unwrap();
        match stmt {
            Statement::Fetch(f) => {
                assert!(f.filter.is_some());
                assert_eq!(f.order_by.len(), 1);
    ...
    fn parse_search_with_no_prefilter_hint() {
        let stmt =
            parse("SEARCH /*+ NO_PREFILTER */ venues NEAR VECTOR [1.0, 0.0] LIMIT 10;").unwrap();
        match stmt {
            Statement::Search(s) => {
    ...
    fn parse_hint_timeout() {
        let stmt =
            parse("SEARCH /*+ TIMEOUT(5000) */ venues NEAR VECTOR [1.0, 0.0] LIMIT 5;").unwrap();
        match stmt {
            Statement::Search(s) => {
    ...
    #[test]
    fn parse_left_join() {
        let stmt = parse("FETCH * FROM entities AS e LEFT JOIN venues AS v ON e.venue_id = v.id;")
            .unwrap();
        match stmt {
            Statement::FetchJoin(j) => {
    ...
    #[test]
    fn parse_right_join() {
        let stmt = parse("FETCH * FROM entities AS e RIGHT JOIN venues AS v ON e.venue_id = v.id;")
            .unwrap();
        match stmt {
            Statement::FetchJoin(j) => {
    ...
    #[test]
    fn parse_full_join() {
        let stmt = parse("FETCH * FROM entities AS e FULL JOIN venues AS v ON e.venue_id = v.id;")
            .unwrap();
        match stmt {
            Statement::FetchJoin(j) => {
    ...
    fn parse_traverse_match_undirected() {
        let stmt = parse("TRAVERSE FROM 'x' MATCH (a)-[e:KNOWS]-(b) DEPTH 1..2;").unwrap();
        match stmt {
            Statement::TraverseMatch(t) => {
    ...
    fn parse_traverse_match_no_edge_type() {
        let stmt = parse("TRAVERSE FROM 'x' MATCH (a)-[e]->(b) DEPTH 1..5;").unwrap();
        match stmt {
            Statement::TraverseMatch(t) => {
                assert_eq!(t.pattern.edge.variable, Some("e".into()));
    ...
    fn parse_traverse_match_with_limit() {
        let stmt =
            parse("TRAVERSE FROM 'x' MATCH (a)-[e:KNOWS]->(b) DEPTH 1..3 LIMIT 10;").unwrap();
        match stmt {
            Statement::TraverseMatch(t) => {
                assert_eq!(t.limit, Some(10));
            }
    ...
    fn parse_fetch_as_of() {
        let stmt =
            parse("FETCH * FROM entities AS OF '2025-01-01T00:00:00Z' WHERE id = 'ent_abc123';")
                .unwrap();
        match stmt {
            Statement::Fetch(f) => {
    ...
    fn parse_fetch_as_of_transaction() {
        let stmt = parse("FETCH * FROM entities AS OF TRANSACTION 42891 WHERE id = 'ent_abc123';")
            .unwrap();
        match stmt {
            Statement::Fetch(f) => {
    ...
    fn parse_traverse_match_as_of() {
        ...
        match stmt {
            Statement::TraverseMatch(t) => {
                assert_eq!(
                    t.temporal,
                    Some(TemporalClause::AsOf("2025-06-01T00:00:00Z".into()))
                );
            }
            _ => panic!("expected TraverseMatch"),
        }
    ...
    fn parse_traverse_match_no_temporal() {
        let stmt =
            parse("TRAVERSE FROM 'ent_abc123' MATCH (a)-[e:RELATED_TO]->(b) DEPTH 1..3;").unwrap();
        match stmt {
            Statement::TraverseMatch(t) => {
    ...
    fn parse_insert_no_temporal() {
        let stmt = parse("INSERT INTO events (id, name) VALUES ('evt1', 'Concert');").unwrap();
        match stmt {
            Statement::Insert(i) => {
    ...
    fn parse_upsert_basic() {
        let stmt = parse("INSERT OR UPDATE INTO venues (id, name) VALUES ('v1', 'Updated Venue');")
            .unwrap();
        match stmt {
            Statement::Upsert(u) => {
                assert_eq!(u.collection, "venues");
