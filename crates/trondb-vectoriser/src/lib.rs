            ...
            #[cfg(feature = "external")]
            {
                let endpoint =
                    config
                        .endpoint
                        .as_deref()
                        .ok_or(VectoriserError::ModelNotLoaded(
                            "external vectoriser requires ENDPOINT".into(),
                        ))?;
                ...
                Ok(Arc::new(external::ExternalVectoriser::new(
                    model, endpoint, auth, dims,
                )))
            }
            #[cfg(not(feature = "external"))]
            Err(VectoriserError::NotSupported(
                "external feature not enabled".into(),
            ))
        }
        Some("network") => {
            let endpoint = config
                ...
                .as_deref()
                .ok_or(VectoriserError::ModelNotLoaded(
                    "network vectoriser requires ENDPOINT".into(),
                ))?;
            let model = config.model.as_deref().unwrap_or("unknown");
            let dims = repr.dimensions.unwrap_or(0);
            Ok(Arc::new(network::NetworkVectoriser::new(
                model, endpoint, dims,
            )))
        }
        _ => {
            #[cfg(feature = "onnx")]
            {
                let model_path =
                    config
                        ...
                        .as_deref()
                        .ok_or(VectoriserError::ModelNotLoaded(
                            "local vectoriser requires MODEL_PATH".into(),
                        ))?;
                let model = config.model.as_deref().unwrap_or("unknown");
                let dims = repr.dimensions.unwrap_or(0);
...
fn merge_configs(
    ...
    trondb_core::types::VectoriserConfig {
        ...
        vectoriser_type: p
            .vectoriser_type
            .clone()
            .or_else(|| f.vectoriser_type.clone()),
