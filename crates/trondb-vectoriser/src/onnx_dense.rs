...
impl OnnxDenseVectoriser {
    pub fn new(
        ...
        let builder =
            Session::builder().map_err(|e| VectoriserError::ModelNotLoaded(e.to_string()))?;
    ...
    fn encode_text(&self, text: &str) -> Result<Vec<f32>, VectoriserError> {
        ...
        let vector = if dims_slice.len() == 3 {
            ...
            return Err(VectoriserError::EncodeFailed(format!(
                "unexpected output shape: {dims_slice:?}"
            )));
        };
...
impl Vectoriser for OnnxDenseVectoriser {
    ...
    async fn encode_batch(&self, batch: &[FieldSet]) -> Result<Vec<VectorData>, VectoriserError> {
        ...
        let all_types = vec![0i64; batch_size * max_seq];
        ...
        if dims.len() == 3 {
            ...
            return Err(VectoriserError::EncodeFailed(format!(
                "unexpected batch output shape: {dims:?}"
            )));
        }
...
mod tests {
    ...
    async fn onnx_dense_encode() {
        ...
        let v = OnnxDenseVectoriser::new("bge-small-en-v1.5", &model_path, &tokenizer_path, 384)
            .unwrap();
