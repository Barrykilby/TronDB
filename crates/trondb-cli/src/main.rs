...
async fn run_remote(addr: &str) {
    ...
    let mut client =
        match trondb_proto::pb::tron_node_client::TronNodeClient::connect(endpoint.clone()).await {
            Ok(c) => c
...
async fn run_local() {
    ...
    let lru = Arc::new(std::sync::Mutex::new(
        trondb_routing::eviction::LruTracker::new(),
    ));
...
fn register_vectorisers_for_schema(
    ...
    for repr in &schema.representations {
        if !repr.fields.is_empty() && (repr.vectoriser.is_some() || coll_config.is_some()) {
            match trondb_vectoriser::create_vectoriser_from_config(
                repr.vectoriser.as_ref(),
                coll_config,
                repr,
            ) {
