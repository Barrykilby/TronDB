...
pub fn format_result(result: &QueryResult) -> String {
    ...
    for row in &result.rows {
        let cells: Vec<String> = result
            ...
            .map(|col| {
                if col == "_score" {
                    row.score.map(|s| format!("{s:.4}")).unwrap_or_default()
                } else {
                    row.values
                        .get(col)
                        .map(|v| v.to_string())
...
pub fn format_grpc_response(resp: &pb::QueryResponse) -> String {
    ...
    if resp.columns.is_empty() && resp.rows.is_empty() {
        ...
        if let Some(ref stats) = resp.stats {
            return format!("OK | {:.2}ms", stats.elapsed_nanos as f64 / 1_000_000.0,);
        }
        return "OK".to_string();
    }
        ...
        let cells: Vec<String> = resp
            ...
            .map(|col| {
                if col == "_score" {
                    ...
                    row.values.get(col).map(format_value).unwrap_or_default()
                }
            })
            .collect();
        table.add_row(cells);
