...
mod tests {
    ...
    async fn make_engine(dir: &std::path::Path) -> (Engine, Vec<WalRecord>) {
    ...
    async fn process_pending_affinity_records() {
        use crate::affinity::{AffinityGroupCreatePayload, AffinityGroupMemberPayload};
        ...
        assert_eq!(affinity.group_for(&LogicalId::from_string("e1")), Some(gid),);
    }
    ...
    async fn process_pending_tier_migration_re_execute() {
        ...
        assert!(
            source.is_none(),
            "source tier should be cleared after re-execution"
        );
        ...
        assert!(
            target.is_some(),
            "target tier should have data after re-execution"
        );
    }
