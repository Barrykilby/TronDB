...
pub use eviction::{eviction_priority, select_eviction_candidates, EvictionPriority, LruTracker};
pub use health::{compute_load_score, HealthCache, HealthSignal, NodeStatus};
pub use node::{
    AffinityGroupId, EntityId, LocalNode, NodeHandle, NodeId, NodeRole, QueryVerb, RoutingStrategy,
    SimulatedNode,
};
pub use router::{
    entity_affinity_score, estimate_acu, plan_targets, plan_verb, routing_score, verb_fit_score,
...
mod tests {
    ...
    async fn router_single_node_routes_to_only_node() {
        ...
        let node = std::sync::Arc::new(node::LocalNode::new(
            engine,
            node::NodeId::from_string("local"),
    ...
    async fn explain_includes_routing_section() {
        ...
        let has_node = result.rows.iter().any(|r| {
            r.values.get("property")
                == Some(&trondb_core::types::Value::String("selected_node".into()))
        });
    ...
    async fn multi_node_routing_prefers_healthy_node() {
        ...
        let node1 = std::sync::Arc::new(node::SimulatedNode::new(
            engine1,
            node::NodeId::from_string("node-1"),
        )) as std::sync::Arc<dyn node::NodeHandle>;
        let node2 = std::sync::Arc::new(node::SimulatedNode::new(
            engine2,
            node::NodeId::from_string("node-2"),
        )) as std::sync::Arc<dyn node::NodeHandle>;

        let router =
            router::SemanticRouter::new(vec![node1, node2], config::RouterConfig::default());

        // Manually seed health cache: node-1 healthy (0.2), node-2 stressed (0.7)
        router
            .health_cache()
            .update(make_health_signal("node-1", 0.2));
        router
            .health_cache()
            .update(make_health_signal("node-2", 0.7));
        ...
        let decision = router
            ...
            .expect("should have a routing decision");
        assert_eq!(
            decision.node,
            node::NodeId::from_string("node-1"),
    ...
    async fn load_shedding_avoids_overloaded_node() {
        ...
        router
            .health_cache()
            .update(make_health_signal("node-1", 0.9));
        router
            .health_cache()
            .update(make_health_signal("node-2", 0.3));
    ...
    async fn all_nodes_overloaded_returns_cluster_overloaded() {
        ...
        router
            .health_cache()
            .update(make_health_signal("node-2", 0.9));
        ...
        assert!(
            ...
            "expected ClusterOverloaded, got {result:?}"
        );
    }
    ...
    fn cooccurrence_learning_creates_implicit_groups() {
        ...
        assert_eq!(
            group_a, group_b,
            "both entities should be in the same learned group"
        );
    ...
    fn affinity_group_crud() {
        ...
        let group = idx.get_group(&gid).unwrap();
        assert!(
            !group.members.contains(&entity),
            "group members should not contain removed entity"
        );
        ...
        assert!(
            ...
            "adding to full group should return AffinityGroupFull, got {err:?}"
        );
    }
    ...
    async fn make_tier_test_setup(
        dir: &std::path::Path,
    ) -> (std::sync::Arc<trondb_core::Engine>, router::SemanticRouter) {
        ...
        let mut router = router::SemanticRouter::new(vec![node], config::RouterConfig::default());
    ...
    async fn explain_tiers_shows_distribution() {
        ...
        assert!(
            tier_names.contains(&"Warm".to_string()),
            "missing Warm tier"
        );
        assert!(
            tier_names.contains(&"Archive".to_string()),
            "missing Archive tier"
        );
    ...
    async fn demote_moves_entity_to_warm() {
        ...
        // DEMOTE v1 to warm
        let plan = engine
            .parse_and_plan("DEMOTE 'v1' FROM venues TO WARM;")
            .unwrap();
        let result = router.route_and_execute(&plan).await.unwrap();
        assert_eq!(
        ...
        assert_eq!(
            hot_count, 0,
            "hot tier should have 0 entities after demotion"
        );
        assert_eq!(
            warm_count, 1,
            "warm tier should have 1 entity after demotion"
        );
    }
    ...
    async fn promote_moves_entity_from_warm_to_hot() {
        ...
        assert_eq!(
            hot_count, 1,
            "hot tier should have 1 entity after promotion"
        );
        assert_eq!(
            warm_count, 0,
            "warm tier should have 0 entities after promotion"
        );
    }
    ...
    async fn affinity_group_records_returned_as_unhandled() {
        ...
        let affinity_records: Vec<_> = pending
            .iter()
            .filter(|r| {
                matches!(
                    r.record_type,
            ...
            .collect();
        assert!(
            !affinity_records.is_empty(),
            "AffinityGroupCreate should be in unhandled records"
        );
    }
