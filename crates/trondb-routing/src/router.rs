...
impl SemanticRouter {
    pub fn new(nodes: Vec<Arc<dyn NodeHandle>>, config: RouterConfig) -> Self {
    ...
    pub async fn route_and_execute(&self, plan: &Plan) -> Result<QueryResult, RouterError> {
        ...
        match plan {
            ...
            Plan::Demote(ref d) => {
                ...
                return Ok(trondb_core::result::QueryResult {
                    ...
                    rows: vec![trondb_core::result::Row {
                        values: HashMap::from([(
                            "status".into(),
                            trondb_core::types::Value::String("OK".into()),
                        )]),
                        score: None,
                    }],
            ...
            Plan::Promote(ref p) => {
                ...
                if let Some(migrator) = &self.migrator {
                    let warm_data = migrator
                        .engine
                        .read_tiered(&p.collection, &entity_id, trondb_core::location::Tier::NVMe)
                    ...
                    if let Some(data) = warm_data {
                        ...
                        migrator
                            .promote_to_hot(&p.collection, &entity_id, vector)
                            .await?;
                    } else {
                        return Err(RouterError::Engine(
                            trondb_core::error::EngineError::EntityNotFound(p.entity_id.clone()),
                        ));
                    }
                }
...
mod tests {
    ...
    fn verb_fit_search_prefers_low_hnsw_latency() {
        ...
        assert!(
            verb_fit_score(QueryVerb::Search, &fast) > verb_fit_score(QueryVerb::Search, &slow)
        );
    }
