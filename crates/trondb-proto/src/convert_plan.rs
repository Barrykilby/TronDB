...
fn where_clause_to_proto(clause: &WhereClause) -> pb::WhereClauseProto {
    ...
    pb::WhereClauseProto {
        clause: Some(match clause {
            ...
            WhereClause::Neq(f, _) => {
                panic!("WHERE != on field '{f}' is not yet supported over gRPC transport")
            }
            WhereClause::Not(_) => panic!("WHERE NOT is not yet supported over gRPC transport"),
            WhereClause::IsNull(f) => {
                panic!("WHERE IS NULL on field '{f}' is not yet supported over gRPC transport")
            }
            WhereClause::IsNotNull(f) => {
                panic!("WHERE IS NOT NULL on field '{f}' is not yet supported over gRPC transport")
            }
            WhereClause::In(f, _) => {
                panic!("WHERE IN on field '{f}' is not yet supported over gRPC transport")
            }
            WhereClause::Like(f, _) => {
                panic!("WHERE LIKE on field '{f}' is not yet supported over gRPC transport")
            }
        }),
    }
}
...
fn proto_to_where_clause(proto: &pb::WhereClauseProto) -> Result<WhereClause, String> {
    ...
    let extract_bin =
        |bin: &pb::BinaryClause| -> Result<(Box<WhereClause>, Box<WhereClause>), String> {
            let left =
                proto_to_where_clause(bin.left.as_ref().ok_or("missing binary left")?.as_ref())?;
            let right =
                proto_to_where_clause(bin.right.as_ref().ok_or("missing binary right")?.as_ref())?;
...
fn vector_literal_to_proto(vl: &VectorLiteral) -> pb::VectorLiteralProto {
    ...
    pb::VectorLiteralProto {
        vector: Some(match vl {
            VectorLiteral::Dense(v) => Vector::Dense(pb::DenseVector { values: v.clone() }),
...
fn temporal_clause_to_proto(tc: &TemporalClause) -> pb::TemporalClauseProto {
    ...
    pb::TemporalClauseProto {
        clause: Some(match tc {
            ...
            TemporalClause::ValidDuring(start, end) => Clause::ValidDuring(pb::ValidDuringProto {
                start: start.clone(),
                end: end.clone(),
            }),
...
fn fetch_strategy_to_proto(fs: &FetchStrategy) -> (i32, String) {
    match fs {
        FetchStrategy::FullScan => (
            pb::FetchStrategyProto::FetchStrategyFullScan as i32,
            String::new(),
        ),
        FetchStrategy::FieldIndexLookup(name) => (
            pb::FetchStrategyProto::FetchStrategyFieldIndexLookup as i32,
            name.clone(),
        ),
        FetchStrategy::FieldIndexRange(name) => (
            pb::FetchStrategyProto::FetchStrategyFieldIndexRange as i32,
            name.clone(),
        ),
    }
}
...
fn field_decl_to_proto(fd: &FieldDecl) -> pb::FieldDecl {
    let (ft, entity_ref) = match &fd.field_type {
        ...
        FieldType::EntityRef(coll) => (
            pb::FieldTypeProto::FieldTypeEntityRef as i32,
            Some(coll.clone()),
        ),
    };
    pb::FieldDecl {
...
fn proto_to_field_decl(proto: &pb::FieldDecl) -> Result<FieldDecl, String> {
    let field_type = match proto.field_type {
        ...
        x if x == pb::FieldTypeProto::FieldTypeEntityRef as i32 => FieldType::EntityRef(
            proto
...
fn proto_to_pre_filter(proto: &pb::PreFilterProto) -> Result<PreFilter, String> {
    Ok(PreFilter {
        ...
        clause: proto_to_where_clause(proto.clause.as_ref().ok_or("missing pre_filter clause")?)?,
    })
}
...
fn proto_to_edge_direction(val: i32) -> trondb_tql::EdgeDirection {
    match val {
        x if x == pb::EdgeDirectionProto::EdgeDirectionBackward as i32 => {
            trondb_tql::EdgeDirection::Backward
        }
        x if x == pb::EdgeDirectionProto::EdgeDirectionUndirected as i32 => {
...
fn proto_to_traverse_match_plan(
    p: &pb::TraverseMatchPlanProto,
) -> Result<TraverseMatchPlan, String> {
            ...
            plan: Some(match plan {
                ...
                Plan::Join(p) => {
                    ...
                    let joins = p
                        .joins
                        .iter()
                        .map(|j| pb::JoinClauseProto {
                            join_type: match j.join_type {
                                trondb_tql::JoinType::Inner => {
                                    pb::JoinTypeProto::JoinTypeInner.into()
                                }
                                trondb_tql::JoinType::Left => {
                                    pb::JoinTypeProto::JoinTypeLeft.into()
                                }
                                trondb_tql::JoinType::Right => {
                                    pb::JoinTypeProto::JoinTypeRight.into()
                                }
                                trondb_tql::JoinType::Full => {
                                    pb::JoinTypeProto::JoinTypeFull.into()
                                }
                            },
                ...
                Plan::TraverseMatch(p) => PP::TraverseMatch(traverse_match_plan_to_proto(p)),

                Plan::Checkpoint(_) => PP::Checkpoint(pb::CheckpointPlanProto {}),

                Plan::Upsert(p) => PP::Upsert(pb::UpsertPlanProto {
                ...
                Plan::Backup(p) => PP::Backup(pb::BackupPlanProto {
                    path: p.path.clone(),
                }),

                Plan::Restore(p) => PP::Restore(pb::RestorePlanProto {
                    path: p.path.clone(),
                }),
                ...
                Plan::AlterCollection(p) => {
                    ...
                    let operation = match &p.operation {
                        ...
                        AlterCollectionOp::SetModel { model, model_path } => Some(
                            pb::alter_collection_plan_proto::Operation::SetModel(pb::SetModelOp {
                                model: model.clone(),
                                model_path: model_path.clone(),
                            }),
                        ),
                        AlterCollectionOp::AlterRepresentationSetModel {
                            repr_name,
                            model,
                            model_path,
                        ...
                        AlterCollectionOp::AddRepresentation {
                            name,
                            dimensions,
                            metric,
                            sparse,
                            ...
                            Some(
                                pb::alter_collection_plan_proto::Operation::AddRepresentation(
                ...
                Plan::Import(p) => PP::ImportPlan(pb::ImportPlanProto {
                    collection: p.collection.clone(),
                    path: p.path.clone(),
                }),
            }),
...
impl TryFrom<pb::PlanRequest> for Plan {
    ...
    fn try_from(proto: pb::PlanRequest) -> Result<Self, String> {
        ...
        match proto.plan.ok_or("missing plan")? {
            ...
            PP::Search(sp) => {
                ...
                Ok(Plan::Search(SearchPlan {
                    ...
                    within: sp
                        .within
                        .as_ref()
                        .map(proto_to_traverse_match_plan)
                        .transpose()?
                        .map(Box::new),
                }))
            }
            ...
            PP::DropCollection(p) => Ok(Plan::DropCollection(DropCollectionPlan { name: p.name })),

            PP::DropEdgeType(p) => Ok(Plan::DropEdgeType(DropEdgeTypePlan { name: p.name })),
                ...
                let fields = if fields_proto.all {
                    ...
                    trondb_tql::JoinFieldList::Named(
                        fields_proto
                            .names
                            .iter()
                            .map(|qf| trondb_tql::QualifiedField {
                                alias: qf.alias.clone(),
                                field: qf.field.clone(),
                            })
                            .collect(),
            ...
            PP::TraverseMatch(p) => Ok(Plan::TraverseMatch(proto_to_traverse_match_plan(&p)?)),
            ...
            PP::Backup(p) => Ok(Plan::Backup(BackupPlan { path: p.path })),

            PP::Restore(p) => Ok(Plan::Restore(RestorePlan { path: p.path })),
...
mod tests {
    ...
    fn round_trip_all_simple_plans() {
        ...
        // CreateAffinityGroup
        let plan = Plan::CreateAffinityGroup(CreateAffinityGroupPlan { name: "g1".into() });
    ...
    fn round_trip_create_collection() {
        let plan = Plan::CreateCollection(CreateCollectionPlan {
            ...
            indexes: vec![IndexDecl {
                ...
                partial_condition: Some(WhereClause::Eq("active".into(), Literal::Bool(true))),
            }],
        ...
        match restored {
            Plan::CreateCollection(cp) => {
                ...
                assert_eq!(
                    cp.representations[0].model,
                    Some("text-embedding-3-small".into())
                );
                assert_eq!(cp.fields.len(), 3);
                assert!(
                    matches!(cp.fields[2].field_type, FieldType::EntityRef(ref c) if c == "other")
                );
    ...
    fn fetch_plan_temporal_as_of_round_trip() {
        ...
        match restored {
            Plan::Fetch(f) => {
                assert_eq!(
                    f.temporal,
                    Some(TemporalClause::AsOf("2025-01-01T00:00:00Z".into()))
                );
            }
            _ => panic!("expected Fetch"),
        }
    ...
    fn fetch_plan_temporal_valid_during_round_trip() {
        let plan = Plan::Fetch(FetchPlan {
            ...
            fields: FieldList::All,
            filter: None,
            temporal: Some(TemporalClause::ValidDuring(
                "2024-01-01".into(),
                "2024-12-31".into(),
            )),
            order_by: vec![],
        ...
        match restored {
            Plan::Fetch(f) => {
                assert_eq!(
                    f.temporal,
                    Some(TemporalClause::ValidDuring(
                        "2024-01-01".into(),
                        "2024-12-31".into()
                    ))
                );
    ...
    fn traverse_match_temporal_round_trip() {
        ...
        match restored {
            Plan::TraverseMatch(tp) => {
                assert_eq!(
                    tp.temporal,
                    Some(TemporalClause::AsOf("2024-08-01T00:00:00Z".into()))
                );
            }
            _ => panic!("expected TraverseMatch"),
        }
