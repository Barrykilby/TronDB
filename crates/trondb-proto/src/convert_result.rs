...
fn proto_to_plan_warning(proto: &pb::PlanWarningProto) -> PlanWarning {
    ...
    PlanWarning {
        ...
        suggestion: if proto.suggestion.is_empty() {
            None
        } else {
            Some(proto.suggestion.clone())
        },
        acu_impact: if proto.acu_impact == 0.0 {
            None
        } else {
            Some(proto.acu_impact)
        },
    }
}
...
impl TryFrom<pb::QueryResponse> for QueryResult {
    ...
    fn try_from(proto: pb::QueryResponse) -> Result<Self, Self::Error> {
        ...
        let stats = QueryStats {
            ...
            warnings: stats_proto
                .warnings
                .iter()
                .map(proto_to_plan_warning)
                .collect(),
        };
...
mod tests {
    ...
    fn plan_warning_round_trip() {
        ...
        assert_eq!(
            restored.suggestion.as_deref(),
            Some("add an index on 'city'")
        );
    ...
    fn plan_warning_critical_round_trip() {
        let warning = PlanWarning::critical("archive-only results").with_acu_impact(100.0);
    ...
    fn query_result_with_cost_and_warnings_round_trip() {
        ...
        let warnings = vec![
            PlanWarning::warning("large result set (k=100)").with_acu_impact(100.0),
        ...
        let result = QueryResult {
            ...
            rows: vec![Row {
                values: HashMap::from([("name".into(), Value::String("Alice".into()))]),
                score: Some(0.95),
            }],
            stats: QueryStats {
                elapsed: Duration::from_millis(150),
        ...
        assert_eq!(
            restored.stats.warnings[0].severity,
            WarningSeverity::Warning
        );
        assert!(restored.stats.warnings[0]
            .message
            .contains("large result set"));
