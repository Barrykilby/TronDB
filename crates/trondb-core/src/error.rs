...
mod tests {
    ...
    fn error_display_messages() {
        assert!(EngineError::DuplicateIndex("idx1".into())
            .to_string()
            .contains("idx1"));
        assert!(EngineError::DuplicateRepresentation("identity".into())
            .to_string()
            .contains("identity"));
        assert!(EngineError::DuplicateField("status".into())
            .to_string()
            .contains("status"));
        assert!(EngineError::FieldNotIndexed("city".into())
            .to_string()
            .contains("city"));
        assert!(EngineError::SparseVectorRequired("venues".into())
            .to_string()
            .contains("venues"));
