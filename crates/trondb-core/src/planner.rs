...
use trondb_tql::{
    FieldList, Literal, OrderByClause, QueryHint, Statement, VectorLiteral, WhereClause,
};
...
pub fn plan(
    ...
    match stmt {
        ...
        Statement::CreateEdgeType(s) => {
            let inference_config = s.inference_config.as_ref().map(|ic| InferenceConfig {
                auto: ic.auto,
        ...
        Statement::Backup(s) => Ok(Plan::Backup(BackupPlan {
            path: s.path.clone(),
        })),
        Statement::Restore(s) => Ok(Plan::Restore(RestorePlan {
            path: s.path.clone(),
        })),
...
pub fn estimate_plan_cost(
    ...
    match plan {
            ...
            match &p.strategy {
                ...
                SearchStrategy::NaturalLanguage => {
                    est.add(
                        ...
                        1,
                        cost.search_natural_language_acu(),
                    );
                }
            }
        ...
        Plan::Infer(_) => AcuEstimate::single("infer", 1, cost.infer_acu()),
        Plan::Join(p) => {
        ...
        // Write operations
        Plan::Insert(_)
        | Plan::CreateCollection(_)
        | Plan::CreateEdgeType(_)
        | Plan::InsertEdge(_)
        | Plan::DeleteEntity(_)
        | Plan::DeleteEdge(_)
        | Plan::CreateAffinityGroup(_)
        | Plan::AlterEntityDropAffinity(_)
        | Plan::Demote(_)
        | Plan::Promote(_)
        | Plan::UpdateEntity(_)
        | Plan::ConfirmEdge(_)
        | Plan::DropCollection(_)
        | Plan::DropEdgeType(_)
        | Plan::Upsert(_)
        | Plan::Backup(_)
        | Plan::AlterCollection(_)
        | Plan::Import(_) => AcuEstimate::single("write", 1, cost.write_base_acu()),
        // Metadata / explain operations are free
        Plan::Explain(_)
        | Plan::ExplainTiers(_)
        | Plan::ExplainHistory(_)
        | Plan::Checkpoint(_)
        | Plan::Restore(_) => AcuEstimate::zero(),
    }
}
...
mod tests {
    ...
    fn plan_search_where_with_indexed_field() {
        use crate::types::{FieldType, Metric, StoredField, StoredIndex, StoredRepresentation};
    ...
    fn plan_fetch_with_indexed_field() {
        ...
        match p {
            Plan::Fetch(fp) => {
                assert_eq!(
                    fp.strategy,
                    FetchStrategy::FieldIndexLookup("idx_city".into())
                );
                assert_eq!(fp.collection, "venues");
            }
            _ => panic!("expected FetchPlan"),
    ...
    fn plan_fetch_gt_uses_field_index_range() {
        ...
        match p {
            Plan::Fetch(fp) => {
                assert_eq!(
                    fp.strategy,
                    FetchStrategy::FieldIndexRange("idx_score".into())
                );
            }
            _ => panic!("expected FetchPlan"),
        }
    ...
    fn plan_update_entity() {
        use trondb_tql::{Literal, UpdateStmt};
    ...
    fn plan_search_no_prefilter_suppresses_prefilter() {
        ...
        match p {
            Plan::Search(sp) => {
                assert!(
                    sp.pre_filter.is_none(),
                    "NO_PREFILTER hint should suppress pre-filter"
                );
    ...
    fn plan_traverse_match() {
        use trondb_tql::{EdgeDirection, EdgePattern, MatchPattern, TraverseMatchStmt};
    ...
    fn two_pass_selected_for_large_k() {
        ...
        match p {
            Plan::Search(sp) => {
                assert!(
                    sp.two_pass.is_some(),
                    "two-pass should be selected for k=100"
                );
    ...
    fn two_pass_not_selected_for_small_k() {
        ...
        match p {
            Plan::Search(sp) => {
                assert!(
                    sp.two_pass.is_none(),
                    "two-pass should NOT be selected for k=10"
                );
            }
            _ => panic!("expected SearchPlan"),
        }
    ...
    fn plan_fetch_with_as_of() {
        ...
        let stmt =
            trondb_tql::parse("FETCH * FROM venues AS OF '2025-01-01T00:00:00Z' WHERE id = 'v1';")
                .unwrap();
        let p = plan(&stmt, &schemas).unwrap();
        match p {
            Plan::Fetch(f) => {
                assert_eq!(
                    f.temporal,
                    Some(trondb_tql::TemporalClause::AsOf(
    ...
    fn plan_fetch_with_valid_during() {
        let schemas = empty_schemas();
        let stmt =
            trondb_tql::parse("FETCH * FROM venues VALID DURING '2025-01-01'..'2025-06-30';")
                .unwrap();
        let p = plan(&stmt, &schemas).unwrap();
        match p {
            Plan::Fetch(f) => {
                assert_eq!(
                    f.temporal,
                    Some(trondb_tql::TemporalClause::ValidDuring(
                        "2025-01-01".into(),
                        "2025-06-30".into()
                    ))
                );
    ...
    fn plan_fetch_with_as_of_transaction() {
        let schemas = empty_schemas();
        let stmt = trondb_tql::parse("FETCH * FROM venues AS OF TRANSACTION 42891;").unwrap();
        let p = plan(&stmt, &schemas).unwrap();
        match p {
            Plan::Fetch(f) => {
                assert_eq!(
                    f.temporal,
                    Some(trondb_tql::TemporalClause::AsOfTransaction(42891))
                );
            }
            _ => panic!("expected Fetch plan"),
        }
