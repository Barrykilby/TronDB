...
pub fn save_snapshot(
    ...
    let idmap_bytes =
        bincode::serialize(&idmap).map_err(|e| format!("idmap serialisation failed: {e}"))?;
    ...
    for ext in &["hnsw.graph", "hnsw.data", "hnsw.idmap", "hnsw.meta"] {
        ...
        std::fs::rename(&src, &dst).map_err(|e| format!("atomic rename {ext} failed: {e}"))?;
    }
...
pub fn load_snapshot(
    ...
    let data_bytes = std::fs::read(&data_path).map_err(|e| format!("checksum read data: {e}"))?;
...
mod tests {
    ...
    fn snapshot_save_creates_files() {
        ...
        assert!(dir
            .path()
            .join("test_collection_default.hnsw.graph")
            .exists());
        assert!(dir
            .path()
            .join("test_collection_default.hnsw.data")
            .exists());
        assert!(dir
            .path()
            .join("test_collection_default.hnsw.idmap")
            .exists());
        assert!(dir
            .path()
            .join("test_collection_default.hnsw.meta")
            .exists());
    }
