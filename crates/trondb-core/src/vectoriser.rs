...
mod tests {
    ...
    fn recipe_hash_changes_with_fields() {
        let a = compute_recipe_hash("bge", &["name".into()]);
        let b = compute_recipe_hash("bge", &["name".into(), "description".into()]);
        assert_ne!(a, b);
    }

    #[test]
    fn recipe_hash_is_field_order_independent() {
        let a = compute_recipe_hash("bge", &["name".into(), "description".into()]);
        let b = compute_recipe_hash("bge", &["description".into(), "name".into()]);
        assert_eq!(a, b);
    }

    #[test]
    fn recipe_hash_is_not_all_zeros() {
        let h = compute_recipe_hash("bge", &["name".into()]);
        assert_ne!(h, [0u8; 32]);
    }
}
