...
fn apply_scalar_pre_filter(
    ...
    if let Plan::Search(ref p) = result.plan {
        if p.filter.is_some() && p.pre_filter.is_none() {
            ...
            if scan_cost > 100.0 {
                result.warnings.push(
                    PlanWarning::warning(format!(
                        "SEARCH WHERE without pre-filter: post-filtering {collection_size} entities ({scan_cost:.0} ACU)"
                    ))
            ...
            result
                .rules_applied
                .push("ScalarPreFilter(warn:no_index)".into());
...
fn apply_confidence_pushdown(result: &mut OptimisedPlan, _cost_provider: &dyn CostProvider) {
    if let Plan::Search(ref p) = result.plan {
        if p.confidence_threshold > 0.9 {
            result.warnings.push(PlanWarning::info(format!(
                "high confidence threshold ({:.2}) enables early termination",
                p.confidence_threshold
            )));
            result
                .rules_applied
                .push("ConfidencePushdown(advisory)".into());
        }
    }
}
...
fn apply_traverse_hop_reorder(result: &mut OptimisedPlan, cost_provider: &dyn CostProvider) {
    if let Plan::Traverse(ref p) = result.plan {
        ...
        result
            .rules_applied
            .push("TraverseHopReorder(checked)".into());
    }
}
...
fn apply_on_demand_promotion(result: &mut OptimisedPlan, _cost_provider: &dyn CostProvider) {
    if let Plan::Fetch(ref p) = result.plan {
        ...
        if !has_no_promote {
            ...
            result
                .rules_applied
                .push("OnDemandPromotion(enabled)".into());
        } else {
            result.warnings.push(
                PlanWarning::info(
            ...
            result
                .rules_applied
                .push("OnDemandPromotion(suppressed)".into());
        }
    }
}
...
fn apply_batched_fetch_after_search(result: &mut OptimisedPlan, cost_provider: &dyn CostProvider) {
    if let Plan::Search(ref p) = result.plan {
        ...
        result
            .rules_applied
            .push("BatchedFetchAfterSearch(checked)".into());
    }
}
...
mod tests {
    ...
    use crate::planner::{
        FetchPlan, FetchStrategy, PreFilter, SearchPlan, SearchStrategy, TraversePlan,
    };
    ...
    fn scalar_pre_filter_warns_on_missing_index() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|w| w.message.contains("pre-filter")));
        assert!(result
            .rules_applied
            .iter()
            .any(|r| r.contains("ScalarPreFilter")));
    }
    ...
    fn scalar_pre_filter_no_warn_with_filter() {
        ...
        assert!(result
            .rules_applied
            .iter()
            .any(|r| r.contains("ScalarPreFilter(active)")));
    }
    ...
    fn confidence_pushdown_fires_on_high_threshold() {
        ...
        assert!(result
            .rules_applied
            .iter()
            .any(|r| r.contains("ConfidencePushdown")));
        assert!(result
            .warnings
            .iter()
            .any(|w| w.message.contains("early termination")));
    }
    ...
    fn traverse_warns_on_deep_depth() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|w| w.message.contains("deep TRAVERSE")));
    }
    ...
    fn traverse_no_warn_on_shallow() {
        ...
        assert!(result
            .warnings
            .iter()
            .all(|w| !w.message.contains("deep TRAVERSE")));
    }
    ...
    fn on_demand_promotion_suppressed_by_hint() {
        ...
        assert!(result
            .rules_applied
            .iter()
            .any(|r| r.contains("suppressed")));
        assert!(result
            .warnings
            .iter()
            .any(|w| w.message.contains("NO_PROMOTE")));
    }
    ...
    fn batched_fetch_warns_on_large_k() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|w| w.message.contains("large result set")));
    }
    ...
    fn rules_can_be_disabled() {
        ...
        let config = OptimiserConfig {
            scalar_pre_filter: false,
            confidence_pushdown: false,
            traverse_hop_reorder: false,
            on_demand_promotion: false,
            batched_fetch_after_search: false,
        };
