...
impl FjallStore {
    ...
    pub fn create_collection_schema(&self, schema: &CollectionSchema) -> Result<(), EngineError> {
        ...
        let bytes =
            rmp_serde::to_vec_named(schema).map_err(|e| EngineError::Storage(e.to_string()))?;
    ...
    pub fn insert(&self, collection: &str, entity: Entity) -> Result<(), EngineError> {
        ...
        let bytes =
            rmp_serde::to_vec_named(&entity).map_err(|e| EngineError::Storage(e.to_string()))?;
    ...
    pub fn create_edge_type(&self, edge_type: &crate::edge::EdgeType) -> Result<(), EngineError> {
        ...
        let bytes =
            rmp_serde::to_vec_named(edge_type).map_err(|e| EngineError::Storage(e.to_string()))?;
        self.meta
            .insert(&key, bytes)
            .map_err(|e: fjall::Error| EngineError::Storage(e.to_string()))?;
    ...
    pub fn insert_edge(&self, edge: &crate::edge::Edge) -> Result<(), EngineError> {
        ...
        let bytes =
            rmp_serde::to_vec_named(edge).map_err(|e| EngineError::Storage(e.to_string()))?;
    ...
    pub fn get_edge(
        ...
        from_id: &str,
        to_id: &str,
    ) -> Result<Option<crate::edge::Edge>, EngineError> {
    ...
    pub fn snapshot_to(&self, backup_dir: &std::path::Path) -> Result<usize, EngineError> {
        ...
        for schema in &schemas {
            ...
            let line = serde_json::to_string(&schema_for_export)
                .map_err(|e| EngineError::Storage(e.to_string()))?;
            use std::io::Write;
            writeln!(schema_file, "{line}").map_err(|e| EngineError::Storage(e.to_string()))?;
            total += 1;
        }
        ...
        for schema in &schemas {
            ...
            for entity in &entities {
                let line = serde_json::to_string(entity)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                use std::io::Write;
                writeln!(entity_file, "{line}").map_err(|e| EngineError::Storage(e.to_string()))?;
            }
            total += entities.len();
        }
...
mod tests {
    ...
    fn delete_entity_removes_from_fjall() {
        ...
        store
            .delete_entity("venues", &LogicalId::from_string("e1"))
            .unwrap();
        store.persist().unwrap();
...
mod tier_tests {
    ...
    fn write_and_read_archive_tier() {
        ...
        store
            .write_tiered("venues", &id, Tier::Archive, data)
            .unwrap();
    ...
    fn delete_from_tier() {
        ...
        let id = LogicalId::from_string("e1");
        store
            .write_tiered("venues", &id, Tier::NVMe, b"data")
            .unwrap();
    ...
    fn tier_entity_count() {
        ...
        store
            .write_tiered("venues", &id1, Tier::NVMe, b"data1")
            .unwrap();
        store
            .write_tiered("venues", &id2, Tier::NVMe, b"data2")
            .unwrap();
    ...
    fn tier_partition_names() {
        assert_eq!(
            FjallStore::tier_partition_name("venues", Tier::Fjall),
            "venues"
        );
        assert_eq!(
            FjallStore::tier_partition_name("venues", Tier::NVMe),
            "warm.venues"
        );
        assert_eq!(
            FjallStore::tier_partition_name("venues", Tier::Archive),
            "archive.venues"
        );
    }
}
