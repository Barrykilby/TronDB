...
impl LocationTable {
    ...
    pub fn iter(
        &self,
    ) -> impl Iterator<Item = dashmap::mapref::multiple::RefMulti<'_, ReprKey, LocationDescriptor>>
    {
    ...
    pub fn snapshot(&self, wal_head_lsn: u64) -> Result<Vec<u8>, EngineError> {
        ...
        let body =
            rmp_serde::to_vec_named(&entries).map_err(|e| EngineError::Storage(e.to_string()))?;
    ...
    pub fn restore(bytes: &[u8]) -> Result<(Self, u64), EngineError> {
        ...
        let entries: Vec<(ReprKey, LocationDescriptor)> =
            rmp_serde::from_slice(&bytes[24..]).map_err(|e| EngineError::Storage(e.to_string()))?;
