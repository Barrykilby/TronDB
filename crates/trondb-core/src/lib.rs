...
impl Engine {
    pub async fn open(config: EngineConfig) -> Result<(Self, Vec<WalRecord>), EngineError> {
        ...
        let (location, snap_lsn) = if snap_path.exists() {
            ...
            eprintln!(
                "Location Table: restored {} entries from snapshot (LSN {lsn})",
                lt.len()
            );
            (lt, lsn)
        } else {
        ...
        let executor = Executor::with_data_dir(
            store,
            wal,
            Arc::clone(&location),
            Arc::clone(&vectoriser_registry),
            Arc::clone(&inference_audit),
        ...
        for collection_name in executor.collections() {
            if let Some(schema_ref) = executor.schemas().get(&collection_name) {
                        ...
                        if let Some(dims) = repr.dimensions {
                            ...
                            if !executor.indexes().contains_key(&hnsw_key) {
                                ...
                                if !loaded {
                                    executor
                                        .indexes()
                                        .insert(hnsw_key, crate::index::HnswIndex::new(dims));
                                }
                            }
                        }
                    }
                ...
                for repr in &schema_ref.representations {
                    if repr.sparse {
                        ...
                        executor.sparse_indexes().entry(sparse_key).or_default();
                    }
                }
                ...
                for idx in &schema_ref.indexes {
                    ...
                    if !executor.field_indexes().contains_key(&fidx_key) {
                        if let Ok(partition) = executor
                            .store()
                            .open_field_index_partition(&collection_name, &idx.name)
                        {
                            let field_types: Vec<(String, types::FieldType)> = idx
                                .fields
                                .iter()
                                .filter_map(|f| {
                                    schema_ref
                                ...
                                .collect();
                            executor.field_indexes().insert(
                                fidx_key,
                                crate::field_index::FieldIndex::new(partition, field_types),
                            );
                        }
                    }
                }
            ...
            if let Ok(entities) = executor.scan_collection(&collection_name) {
                for entity in &entities {
                    ...
                    executor
                        .entity_collections()
                        .insert(entity.id.clone(), collection_name.clone());
                    ...
                    for entry in executor.field_indexes().iter() {
                        if entry.key().starts_with(&format!("{collection_name}:")) {
                            let values: Vec<types::Value> = entry
                                .field_types()
                                .iter()
            ...
            for record in &records_to_replay {
                match record.record_type {
                    trondb_wal::RecordType::EntityWrite => {
                        ...
                        let has_snapshot = hnsw_snapshot_lsns
                            .keys()
                            .any(|k| k.starts_with(&format!("{}:", record.collection)));
                        if !has_snapshot {
                            continue;
                        }
                        if let Ok(entity) = rmp_serde::from_slice::<types::Entity>(&record.payload)
                        {
                            for repr in &entity.representations {
                                if let types::VectorData::Dense(ref vec_f32) = repr.vector {
                                    let hnsw_key = format!("{}:{}", record.collection, repr.name);
                    ...
                    trondb_wal::RecordType::EntityDelete => {
                        ...
                        if let Ok(payload) = rmp_serde::from_slice::<DeletePayload>(&record.payload)
                        {
                    ...
                    for entry in indexes_arc.iter() {
                        if let Some((collection, repr_name)) = entry.key().split_once(':') {
                            if let Err(e) = crate::hnsw_snapshot::save_snapshot(
                                ...
                                eprintln!(
                                    "[hnsw_snapshot] periodic save failed for {}: {e}",
                                    entry.key()
                                );
                            }
                        }
    ...
    pub fn parse_and_plan(&self, tql: &str) -> Result<planner::Plan, EngineError> {
        let stmt = trondb_tql::parse(tql).map_err(|e| EngineError::InvalidQuery(e.to_string()))?;
    ...
    pub fn read_tiered(
        ...
        self.executor
            .store()
            .read_tiered(collection, entity_id, tier)
    }
    ...
    pub fn write_tiered(
        ...
        self.executor
            .store()
            .write_tiered(collection, entity_id, tier, data)
    }
    ...
    pub fn delete_from_tier(
        ...
        self.executor
            .store()
            .delete_from_tier(collection, entity_id, tier)
    }
    ...
    pub async fn apply_wal_record(&self, record: &WalRecord) -> Result<(), EngineError> {
        match record.record_type {
            ...
            _ => {
                let (replayed, _unhandled) = self
                    .executor
                    .replay_wal_records(std::slice::from_ref(record))?;
                if replayed > 0 {
...
mod tests {
    ...
    async fn search_returns_results() {
        ...
        assert_eq!(
            ...
            "exact match v1 should rank first"
        );
        // Should have a score
    ...
    async fn insert_with_vector_creates_location_entry() {
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should have location entry");
    ...
    async fn create_edge_type_and_traverse() {
        ...
        engine
            .execute_tql("INSERT INTO people (id, name) VALUES ('p1', 'Alice');")
            .await
            .unwrap();
        engine
            .execute_tql("INSERT INTO people (id, name) VALUES ('p2', 'Bob');")
            .await
            .unwrap();

        engine
            .execute_tql("CREATE EDGE knows FROM people TO people;")
            .await
            .unwrap();
        engine
            .execute_tql("INSERT EDGE knows FROM 'p1' TO 'p2';")
            .await
            .unwrap();

        let result = engine
            .execute_tql("TRAVERSE knows FROM 'p1';")
            .await
            .unwrap();
        assert_eq!(result.rows.len(), 1);
        assert_eq!(
    ...
    async fn delete_edge_removes_from_traverse() {
        ...
        engine
            ...
            .unwrap();
        engine
            .execute_tql("DELETE EDGE knows FROM 'p1' TO 'p2';")
            .await
            .unwrap();
    ...
    async fn insert_edge_with_metadata() {
        ...
        engine
            ...
            .unwrap();
        engine
            .execute_tql("INSERT EDGE knows FROM 'p1' TO 'p2' WITH (since = '2024');")
            .await
            .unwrap();
    ...
    async fn traverse_depth_gt_1_returns_multi_hop_results() {
        ...
        create_simple_collection(&engine, "people", 3).await;
        engine
            .execute_tql("INSERT INTO people (id, name) VALUES ('a', 'Alice');")
            .await
            .unwrap();
        engine
            .execute_tql("INSERT INTO people (id, name) VALUES ('b', 'Bob');")
            .await
            .unwrap();
        engine
            .execute_tql("INSERT INTO people (id, name) VALUES ('c', 'Carol');")
            .await
            .unwrap();
        ...
        engine
            ...
            .unwrap();
        engine
            .execute_tql("INSERT EDGE knows FROM 'a' TO 'b';")
            .await
            .unwrap();
        engine
            .execute_tql("INSERT EDGE knows FROM 'b' TO 'c';")
            .await
            .unwrap();

        // Depth 2 from 'a' should reach both b (hop 1) and c (hop 2)
        let result = engine
            .execute_tql("TRAVERSE knows FROM 'a' DEPTH 2;")
            .await
            .unwrap();
        assert_eq!(result.rows.len(), 2);
    }
    ...
    async fn sparse_index_survives_restart() {
        ...
        // Phase 1: Create collection with sparse repr, insert entity
        {
            ...
            engine
                .execute_tql(
                    "INSERT INTO docs (id, title) VALUES ('d1', 'Hello')
        ...
        // Phase 2: Reopen — sparse index should be rebuilt
        {
            let (engine, _) = Engine::open(config).await.unwrap();
            let result = engine
                .execute_tql("SEARCH docs NEAR SPARSE [1:1.0] LIMIT 5;")
                .await
                .unwrap();
            assert_eq!(result.rows.len(), 1);
            assert_eq!(
    ...
    async fn field_index_survives_restart() {
...

        {
            ...
            engine
                .execute_tql(
                    "INSERT INTO venues (id, city) VALUES ('v1', 'London')
            ...
            engine
                .execute_tql(
                    "INSERT INTO venues (id, city) VALUES ('v2', 'Paris')
                REPRESENTATION identity VECTOR [0.4, 0.5, 0.6];",
                )
                .await
                .unwrap();
        }
    ...
    async fn hybrid_search_works_after_restart() {
...

        {
            ...
            engine
                ...
                REPRESENTATION sparse SPARSE [1:0.8];",
                )
                .await
                .unwrap();
        }

        {
            let (engine, _) = Engine::open(config).await.unwrap();
            let result = engine
                .execute_tql("SEARCH docs NEAR VECTOR [0.1, 0.2, 0.3] NEAR SPARSE [1:1.0] LIMIT 5;")
                .await
                .unwrap();
            assert!(!result.rows.is_empty());
        }
    ...
    async fn create_collection_with_fields_and_indexes() {
        ...
        engine
            .execute_tql(
                "INSERT INTO venues (id, city, active) VALUES ('v1', 'London', true)
            REPRESENTATION identity VECTOR [0.1, 0.2, 0.3];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO venues (id, city, active) VALUES ('v2', 'Paris', true)
            REPRESENTATION identity VECTOR [0.4, 0.5, 0.6];",
            )
            .await
            .unwrap();
    ...
    async fn sparse_search_returns_ranked() {
        ...
        engine
            .execute_tql(
                "INSERT INTO docs (id, title) VALUES ('d1', 'Rust')
            REPRESENTATION keywords SPARSE [1:0.9, 2:0.1];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO docs (id, title) VALUES ('d2', 'Python')
            REPRESENTATION keywords SPARSE [1:0.1, 3:0.8];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO docs (id, title) VALUES ('d3', 'Go')
            REPRESENTATION keywords SPARSE [2:0.5, 3:0.5];",
            )
            .await
            .unwrap();

        let result = engine
            .execute_tql("SEARCH docs NEAR SPARSE [1:1.0] LIMIT 2;")
            .await
            .unwrap();
        assert_eq!(result.rows.len(), 2);
    ...
    async fn hybrid_search_merges_dense_sparse() {
        ...
        engine
            ...
            REPRESENTATION sparse SPARSE [1:0.9];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO docs (id) VALUES ('d2')
            REPRESENTATION dense VECTOR [0.0, 1.0, 0.0]
            REPRESENTATION sparse SPARSE [1:0.1];",
            )
            .await
            .unwrap();
        ...
        let result = engine
            .execute_tql("SEARCH docs NEAR VECTOR [1.0, 0.0, 0.0] NEAR SPARSE [1:1.0] LIMIT 5;")
            .await
            .unwrap();
        assert!(!result.rows.is_empty());
    ...
    async fn scalar_prefilter_narrows_search() {
        ...
        engine
            .execute_tql(
                "INSERT INTO venues (id, city) VALUES ('v1', 'London')
            REPRESENTATION identity VECTOR [1.0, 0.0, 0.0];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO venues (id, city) VALUES ('v2', 'Paris')
            REPRESENTATION identity VECTOR [0.9, 0.1, 0.0];",
            )
            .await
            .unwrap();
        engine
            .execute_tql(
                "INSERT INTO venues (id, city) VALUES ('v3', 'London')
            REPRESENTATION identity VECTOR [0.8, 0.2, 0.0];",
            )
            .await
            .unwrap();
    ...
    async fn error_duplicate_representation() {
        ...
        assert!(
            err_msg.contains("duplicate representation"),
    ...
    async fn error_duplicate_index() {
        ...
        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("duplicate index"),
            "error should mention duplicate: {err_msg}"
        );
    }
    ...
    async fn error_sparse_vector_on_dense_only_collection() {
        ...
        engine
            .execute_tql(
                "INSERT INTO venues (id) VALUES ('v1')
            REPRESENTATION identity VECTOR [0.1, 0.2, 0.3];",
            )
            .await
            .unwrap();
        ...
        let result = engine
            .execute_tql("SEARCH venues NEAR SPARSE [1:1.0] LIMIT 5;")
            .await;
        assert!(result.is_err());
        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("sparse"),
            "error should mention sparse: {err_msg}"
        );
    }
    ...
    async fn error_field_not_indexed() {
        ...
        let result = engine
            .execute_tql("SEARCH venues WHERE city = 'London' NEAR VECTOR [0.1, 0.2, 0.3] LIMIT 5;")
            .await;
        assert!(result.is_err());
        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("not indexed") || err_msg.contains("FieldNotIndexed"),
    ...
    async fn location_table_snapshot_and_restore() {
        ...
        // Reopen — should restore from snapshot
        {
            ...
            let desc = engine
                .location()
                .get(&key)
                .expect("should have location entry after restore");
            assert_eq!(desc.tier, crate::location::Tier::Fjall);
        }
    }
    ...
    async fn update_entity_changes_metadata() {
        ...
        // UPDATE
        engine
            .execute_tql("UPDATE 'v1' IN venues SET name = 'New Name';")
            .await
            .unwrap();

        // FETCH to verify
        let result = engine
            .execute_tql("FETCH * FROM venues WHERE name = 'New Name';")
            .await
            .unwrap();
        ...
        let result = engine
            .execute_tql("FETCH * FROM venues WHERE name = 'Old Name';")
            .await
            .unwrap();
    ...
    async fn update_nonexistent_entity_errors() {
        ...
        let result = engine
            .execute_tql("UPDATE 'nonexistent' IN venues SET name = 'X';")
            .await;
    ...
    async fn wal_replay_entity_delete() {
        ...
        // Insert then delete
        {
            ...
            engine
                ...
                .unwrap();
            engine
                .execute_tql("DELETE 'v1' FROM venues;")
                .await
                .unwrap();
        }
        ...
        assert_eq!(
            result.rows.len(),
            0,
            "entity should not exist after WAL replay of delete"
        );
    }
    ...
    async fn hnsw_snapshot_survives_restart() {
        ...
        assert!(
            ...
            "SEARCH should find entities after snapshot restore"
        );
    }
    ...
    async fn phase8_integration_update_then_restart_with_snapshot() {
        ...
        // Session 1: create, insert, update, snapshot
        {
            ...
            // UPDATE v1's name
            engine
                .execute_tql("UPDATE 'v1' IN venues SET name = 'New';")
                .await
                .unwrap();

            // DELETE v2
            engine
                .execute_tql("DELETE 'v2' FROM venues;")
                .await
                .unwrap();
        ...
        // Session 2: reopen and verify all state survived
        {
            ...
            let result = engine
                .execute_tql("FETCH * FROM venues WHERE name = 'New';")
                .await
                .unwrap();
            assert_eq!(
                result.rows.len(),
                1,
                "v1 should be findable by updated name"
            );

            // Old name should not match
            let result = engine
                .execute_tql("FETCH * FROM venues WHERE name = 'Old';")
                .await
                .unwrap();
            assert_eq!(result.rows.len(), 0, "old name should not match");
            ...
            assert!(
                !result.rows.is_empty(),
                "SEARCH should return results after snapshot restore"
            );
        }
    }
    ...
    async fn hnsw_snapshot_on_drop() {
        ...
        assert!(
            snap_dir.exists(),
            "hnsw_snapshots directory should exist after drop"
        );
        assert!(
            snap_dir.join("venues_default.hnsw.meta").exists(),
    ...
    async fn insert_auto_vectorise_from_fields() {
        ...
        let result = engine
            .execute_tql("FETCH * FROM events WHERE id = 'e1';")
            .await
            .unwrap();
        assert_eq!(result.rows.len(), 1);
        assert_eq!(
    ...
    async fn insert_composite_vector_multiple_fields() {
        ...
        engine
            .vectoriser_registry()
            .register("events", "semantic", mock.clone());
        engine
            .vectoriser_registry()
            .register("events", "name_only", mock);
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "SEARCH should find the auto-vectorised entity"
        );
        ...
        assert_eq!(
            result.rows.len(),
            2,
            "SEARCH should find both auto-vectorised entities"
        );
    }
    ...
    async fn insert_composite_vector_partial_fields() {
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "SEARCH should find entity with partial fields"
        );
    }
    ...
    async fn insert_composite_vector_no_matching_fields_skips() {
        ...
        engine
            .execute_tql("INSERT INTO events (id) VALUES ('e1');")
            .await
            .unwrap();
        ...
        assert_eq!(
            result.rows.len(),
            0,
            "entity with no matching fields should have no vector"
        );
    }
    ...
    async fn update_marks_managed_repr_dirty() {
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should have location entry after INSERT");
        assert_eq!(
            desc.state,
            crate::location::LocState::Clean,
            "should be Clean after INSERT"
        );

        // UPDATE a contributing field
        engine
            .execute_tql("UPDATE 'e1' IN events SET name = 'Blues Night';")
            .await
            .unwrap();
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should still have location entry after UPDATE");
        assert_eq!(
            desc.state,
            crate::location::LocState::Dirty,
            "should be Dirty after UPDATE of contributing field"
        );
    }
    ...
    async fn update_non_contributing_field_stays_clean() {
        ...
        engine
            .execute_tql("UPDATE 'e1' IN events SET category = 'jazz';")
            .await
            .unwrap();
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should still have location entry");
        assert_eq!(
            desc.state,
            crate::location::LocState::Clean,
            "should stay Clean after UPDATE of non-contributing field"
        );
    }
    ...
    async fn update_marks_only_affected_repr_dirty() {
        ...
        engine
            .vectoriser_registry()
            .register("events", "name_repr", mock.clone());
        engine
            .vectoriser_registry()
            .register("events", "desc_repr", mock);
        ...
        engine
            .execute_tql("UPDATE 'e1' IN events SET description = 'Updated description';")
            .await
            .unwrap();

        let desc_name = engine
            .location()
            .get(&key_name)
            .expect("name_repr location");
        let desc_desc = engine
            .location()
            .get(&key_desc)
            .expect("desc_repr location");

        assert_eq!(
            desc_name.state,
            crate::location::LocState::Clean,
            "name_repr should stay Clean"
        );
        assert_eq!(
            desc_desc.state,
            crate::location::LocState::Dirty,
            "desc_repr should be Dirty"
        );
    }
    ...
    async fn dirty_repr_recomputed_to_clean() {
        ...
        // INSERT — auto-vectorise creates entity + location entry (Clean)
        engine
            .execute_tql("INSERT INTO events (id, name) VALUES ('e1', 'Jazz');")
            .await
            .unwrap();
        ...
        engine
            .execute_tql("UPDATE 'e1' IN events SET name = 'Blues';")
            .await
            .unwrap();
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should have location entry after UPDATE");
        assert_eq!(
            desc.state,
            crate::location::LocState::Dirty,
        ...
        let desc = engine
            .location()
            .get(&key)
            .expect("should have location entry after recompute");
        assert_eq!(
            desc.state,
            crate::location::LocState::Clean,
            "should be Clean after recompute"
        );
    }
    ...
    async fn recompute_dirty_handles_multiple_entities() {
        ...
        engine
            ...
            .unwrap();
        engine
            .execute_tql("INSERT INTO events (id, name) VALUES ('e2', 'Rock');")
            .await
            .unwrap();
        ...
        engine
            ...
            .unwrap();
        engine
            .execute_tql("UPDATE 'e2' IN events SET name = 'Metal';")
            .await
            .unwrap();
        ...
        assert_eq!(
            engine.location().get(&key1).unwrap().state,
            crate::location::LocState::Clean
        );
        assert_eq!(
            engine.location().get(&key2).unwrap().state,
            crate::location::LocState::Clean
        );
    }
    ...
    async fn end_to_end_auto_vectorise_and_nl_search() {
        ...
        let result = engine
            .execute_tql("SEARCH venues NEAR 'jazz music' USING semantic LIMIT 10;")
            .await
            .unwrap();
        ...
        engine
            .execute_tql("UPDATE 'v1' IN venues SET name = 'Blue Note Jazz Club';")
            .await
            .unwrap();
        ...
        let result2 = engine
            .execute_tql("SEARCH venues NEAR 'jazz club' USING semantic LIMIT 10;")
            .await
            .unwrap();
        assert!(
            !result2.rows.is_empty(),
            "NL search should return results after recompute"
        );
    }
    ...
    async fn end_to_end_inference_lifecycle() {
        ...
        engine
            .execute_tql("CONFIRM EDGE FROM 'act1' TO 'v1' TYPE performs_at CONFIDENCE 0.90;")
            .await
            .unwrap();
