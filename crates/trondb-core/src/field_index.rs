...
pub fn encode_sortable(value: &Value, field_type: &FieldType) -> Result<Vec<u8>, EngineError> {
    match (value, field_type) {
        ...
        (Value::Bool(b), FieldType::Bool) => Ok(vec![if *b { 0x01 } else { 0x00 }]),
...
impl FieldIndex {
    pub fn new(partition: fjall::PartitionHandle, field_types: Vec<(String, FieldType)>) -> Self {
        Self {
            partition,
            field_types,
        }
    ...
    pub fn insert(&self, entity_id: &LogicalId, values: &[Value]) -> Result<(), EngineError> {
        ...
        self.partition
            .insert(&full_key, entity_id.as_str().as_bytes())
...
mod tests {
    ...
    fn open_test_partition() -> (fjall::PartitionHandle, TempDir) {
        ...
        let part = ks
            .open_partition("test_idx", PartitionCreateOptions::default())
            .unwrap();
        (part, dir)
    }
    ...
    fn field_index_insert_and_lookup_eq() {
        ...
        let idx = FieldIndex::new(partition, vec![("status".into(), FieldType::Text)]);
    ...
    fn field_index_range_lookup() {
        let (partition, _dir) = open_test_partition();
        let idx = FieldIndex::new(partition, vec![("score".into(), FieldType::Int)]);
        for i in 0..10 {
        ...
        let results = idx
            .lookup_range(&[Value::Int(20)], &[Value::Int(50)])
            .unwrap();
    ...
    fn field_index_lookup_eq_wrong_count_fails() {
        ...
        let idx = FieldIndex::new(
            partition,
            vec![("a".into(), FieldType::Text), ("b".into(), FieldType::Text)],
        );
