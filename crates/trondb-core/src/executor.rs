...
use crate::inference::{
    InferenceAuditBuffer, InferenceAuditEntry, InferenceCandidate, InferenceTrigger,
};
use crate::location::{
...
enum SchemaAlterOp {
    ...
    AlterRepresentationSetFields {
        repr_name: String,
        fields: Vec<String>,
    },
    AlterRepresentationSetModel {
        repr_name: String,
        model: String,
        model_path: String,
    },
...
impl Executor {
    ...
    pub fn replay_wal_records(
        &self,
        records: &[WalRecord],
    ) -> Result<(usize, Vec<WalRecord>), EngineError> {
        let mut replayed = 0;
            ...
            match record.record_type {
                RecordType::SchemaCreateColl => {
                    let schema: CollectionSchema = rmp_serde::from_slice(&record.payload)
                        .map_err(|e| EngineError::Storage(
                            format!("failed to deserialize CollectionSchema from WAL: {e}. \
                                     If upgrading from pre-Phase 5a, delete trondb_data/wal/ and restart.")
                        ))?;
                ...
                RecordType::EntityWrite => {
                    ...
                    self.entity_collections
                        .insert(entity.id.clone(), record.collection.clone());
                ...
                RecordType::EntityDelete => {
                    ...
                    let _ =
                        self.store
                            .delete_from_tier(&payload.collection, &entity_id, Tier::NVMe);
                    let _ =
                        self.store
                            .delete_from_tier(&payload.collection, &entity_id, Tier::Archive);
                ...
                RecordType::SchemaCreateEdgeType => {
                    let edge_type: crate::edge::EdgeType =
                        rmp_serde::from_slice(&record.payload)
                            .map_err(|e| EngineError::Storage(e.to_string()))?;
                ...
                RecordType::EdgeDelete => {
                    ...
                    let payload: EdgeDeletePayload = rmp_serde::from_slice(&record.payload)
                        .map_err(|e| EngineError::Storage(e.to_string()))?;
                    self.store
                        .delete_edge(&payload.edge_type, &payload.from_id, &payload.to_id)?;
                    replayed += 1;
                }
                    ...
                    if let Ok(mut schema) = self.store.get_collection_schema(&payload.collection) {
                        match &payload.op {
                            SchemaAlterOp::RenameField { old_name, new_name } => {
                                if let Some(field) =
                                    schema.fields.iter_mut().find(|f| f.name == *old_name)
                                {
                            ...
                            SchemaAlterOp::SetModel { model, model_path } => {
                                let vc = schema.vectoriser_config.get_or_insert(
                                    crate::types::VectoriserConfig {
                                        model: None,
                                        model_path: None,
                                        device: None,
                                        vectoriser_type: None,
                                        endpoint: None,
                                        auth: None,
                                    },
                                );
                            ...
                            SchemaAlterOp::AlterRepresentationSetModel {
                                repr_name,
                                model,
                                model_path,
                            } => {
                                if let Some(repr) = schema
                                    ...
                                    .find(|r| r.name == *repr_name)
                                {
                                    let vc = repr.vectoriser.get_or_insert(
                                        crate::types::VectoriserConfig {
                                            model: None,
                                            model_path: None,
                                            device: None,
                            ...
                            SchemaAlterOp::AddRepresentation {
                                name,
                                dimensions,
                                metric,
                                sparse,
                                ...
                                if !schema.representations.iter().any(|r| r.name == *name) {
                                    schema.representations.push(
                                        crate::types::StoredRepresentation {
                                            name: name.clone(),
                                            model: None,
                                    ...
                                    if *sparse {
                                        ...
                                        self.sparse_indexes.entry(sparse_key).or_default();
        ...
        if resolved.exists() {
            ...
            if !canonical.starts_with(&canonical_base) {
                return Err(EngineError::InvalidQuery(
                    "path outside data directory".into(),
                ));
            }
            Ok(canonical)
        } else {
    ...
    pub async fn execute(&self, plan: &Plan) -> Result<QueryResult, EngineError> {
        ...
        let cost_estimate = match plan {
            ...
            _ => {
                ...
                let size = collection_name
                    ...
                    .unwrap_or(0);
                let est =
                    crate::planner::estimate_plan_cost(plan, self.cost_provider.as_ref(), size);
        ...
        let result = match plan {
            Plan::CreateCollection(p) => {
                ...
                self.wal
                    .append(RecordType::TxBegin, &p.name, tx_id, 1, vec![]);

                let payload = rmp_serde::to_vec_named(&schema)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;

                self.wal
                    .append(RecordType::SchemaCreateColl, &p.name, tx_id, 1, payload);
                ...
                for idx in &schema.indexes {
                    let partition = self
                        .store
                        .open_field_index_partition(&schema.name, &idx.name)?;
                    let field_types: Vec<(String, FieldType)> = idx
                        .fields
                        .iter()
                        .filter_map(|f| {
                            schema
                    ...
                    self.field_indexes
                        .insert(fidx_key, FieldIndex::new(partition, field_types));
                }
            ...
            Plan::Insert(p) => {
                ...
                for (repr_name, vec_lit) in &p.vectors {
                    let schema_repr = schema
                        .representations
                        .iter()
                        .find(|r| r.name == *repr_name)
                        .ok_or_else(|| {
                            EngineError::InvalidQuery(format!(
                                "no representation '{}' in collection '{}'",
                                repr_name, p.collection
                            ))
                        })?;
                ...
                let entity_id = find_id_in_fields(&p.fields, &p.values).unwrap_or_default();
                ...
                if let Ok(old_entity) = self.store.get(&p.collection, &entity_id) {
                    ...
                    for entry in self.field_indexes.iter() {
                        if entry.key().starts_with(&format!("{}:", p.collection)) {
                            let old_values: Vec<Value> = entry
                                .field_types()
                                .iter()
                ...
                for (field, value) in p.fields.iter().zip(p.values.iter()) {
                    ...
                    entity
                        .metadata
                        .insert(field.clone(), literal_to_value(value));
                }
                ...
                for (repr_name, vec_lit) in &p.vectors {
                    let vector_data = match vec_lit {
                        ...
                        VectorLiteral::Sparse(v) => VectorData::Sparse(v.clone()),
                    };

                    // Look up schema representation to get its declared FIELDS list
                    let stored_repr = schema.representations.iter().find(|r| r.name == *repr_name);
                ...
                let explicit_repr_names: HashSet<&str> =
                    p.vectors.iter().map(|(name, _)| name.as_str()).collect();
                ...
                for stored_repr in &schema.representations {
                    ...
                    let vector_data = vectoriser.encode(&field_set).await.map_err(|e| {
                        EngineError::Storage(format!("vectoriser encode failed: {e}"))
                    })?;
                    ...
                    let repr = Representation {
                        ...
                        repr_type: if stored_repr.fields.len() > 1 {
                            ReprType::Composite
                        } else {
                            ReprType::Atomic
                        },
                ...
                self.wal
                    .append(RecordType::TxBegin, &p.collection, tx_id, 1, vec![]);
                ...
                // Track entity → collection mapping (for recompute_dirty)
                self.entity_collections
                    .insert(entity_id.clone(), p.collection.clone());
                ...
                for et in self.edge_types.iter() {
                    if et.value().inference_config.auto
                        && et.value().from_collection == p.collection
                    {
            ...
            Plan::Fetch(p) => {
                ...
                match &p.strategy {
                    FetchStrategy::FieldIndexLookup(index_name) => {
                        ...
                        let fidx = self.field_indexes.get(&fidx_key).ok_or_else(|| {
                            EngineError::InvalidQuery(format!(
                                "field index '{index_name}' not found"
                            ))
                        })?;
                    ...
                    FetchStrategy::FieldIndexRange(index_name) => {
                        ...
                        for eid in &entity_ids {
                            if let Ok(entity) = self.store.get(&p.collection, eid) {
                                ...
                                if p.filter
                                    .as_ref()
                                    .map(|c| entity_matches(&entity, c))
                                    .unwrap_or(true)
                                {
                                    // Apply temporal filter
                                    if let Some(ref tc) = p.temporal {
                                        if !entity_passes_temporal_filter(&entity, tc)
                                            .unwrap_or(false)
                                        {
                                            continue;
                                        }
                    ...
                    FetchStrategy::FullScan => {
                        ...
                        let entities = if p.filter.is_none()
                            && p.temporal.is_none()
                            && p.order_by.is_empty()
                            && p.limit.is_some()
                        {
                        ...
                        let filtered: Vec<&Entity> = entities
                            ...
                            .filter(|e| match &p.temporal {
                                Some(clause) => {
                                    entity_passes_temporal_filter(e, clause).unwrap_or(false)
                                }
                                None => true,
                            })
                            .collect();
            ...
            Plan::Search(p) => {
                ...
                if let Some(ref within_plan) = p.within {
                    ...
                    let within_ids: HashSet<LogicalId> = traverse_result
                        .rows
                        .iter()
                        .filter_map(|row| {
                            row.values.get("id").map(|v| match v {
                ...
                let fetch_k = if pre_filter_ids.is_some() {
                    p.k * 4
                } else {
                    p.k
                };
                ...
                let results: Vec<(LogicalId, f32)> = match &p.strategy {
                    SearchStrategy::Hnsw => {
                        let query = p.dense_vector.as_ref().ok_or_else(|| {
                            EngineError::InvalidQuery("HNSW requires dense vector".into())
                        })?;
                        ...
                        let hnsw_key = self
                            ...
                            .map(|e| e.key().clone())
                            .ok_or_else(|| {
                                EngineError::InvalidQuery("no HNSW index for collection".into())
                            })?;
                    ...
                    SearchStrategy::Sparse => {
                        let query = p.sparse_vector.as_ref().ok_or_else(|| {
                            EngineError::InvalidQuery("Sparse requires sparse vector".into())
                        })?;
                        let prefix = format!("{}:", p.collection);
                        let sparse_key = self
                            .sparse_indexes
                            ...
                            .map(|e| e.key().clone())
                            .ok_or_else(|| {
                                EngineError::InvalidQuery("no SparseIndex for collection".into())
                            })?;
                    ...
                    SearchStrategy::Hybrid => {
                        let dense_query = p.dense_vector.as_ref().ok_or_else(|| {
                            EngineError::InvalidQuery("Hybrid requires dense vector".into())
                        })?;
                        let sparse_query = p.sparse_vector.as_ref().ok_or_else(|| {
                            EngineError::InvalidQuery("Hybrid requires sparse vector".into())
                        })?;
                    ...
                    SearchStrategy::NaturalLanguage => {
                        let query_text = p.query_text.as_ref().ok_or_else(|| {
                        ...
                        let repr_name = p
                            ...
                            .ok_or_else(|| {
                                EngineError::InvalidQuery(
                                    "no managed representation found for natural language SEARCH"
                                        .into(),
                                )
                            })?
                            .to_string();
                        ...
                        let vectoriser = self
                            ...
                            .ok_or_else(|| {
                                EngineError::InvalidQuery(format!(
                                    "no vectoriser registered for {}:{}",
                                    p.collection, repr_name
                                ))
                            })?;

                        // Encode query text
                        let query_vector =
                            vectoriser.encode_query(query_text).await.map_err(|e| {
                                EngineError::Storage(format!("encode_query failed: {e}"))
                            })?;
                        ...
                        if is_sparse_repr {
                            ...
                            let sparse_pairs =
                                match query_vector {
                                    ...
                                    _ => return Err(EngineError::InvalidQuery(
                                        "sparse representation vectoriser returned non-sparse data"
                                            .into(),
                                    )),
                                };
                            let sparse_key = format!("{}:{}", p.collection, repr_name);
                            let sparse_idx =
                                self.sparse_indexes.get(&sparse_key).ok_or_else(|| {
                                    EngineError::InvalidQuery(format!(
                            ...
                            let query_f32 =
                                match query_vector {
                                    ...
                                    _ => return Err(EngineError::InvalidQuery(
                                        "dense representation vectoriser returned non-dense data"
                                            .into(),
                                    )),
                                };

                            let hnsw_key = format!("{}:{}", p.collection, repr_name);
                            let hnsw = self.indexes.get(&hnsw_key).ok_or_else(|| {
                                EngineError::InvalidQuery(format!("no HNSW index for {hnsw_key}"))
                            })?;
                ...
                let searched_repr_idx: Option<u32> =
                    self.schemas.get(&p.collection).and_then(|schema| {
                        // Determine which repr_name was searched based on the strategy
                        let prefix = format!("{}:", p.collection);
                        let repr_name = match &p.strategy {
                            SearchStrategy::Hnsw
                            | SearchStrategy::Hybrid
                            | SearchStrategy::NaturalLanguage => self
                                .indexes
                                .iter()
                                .find(|e| e.key().starts_with(&prefix))
                    ...
                    results
                        ...
                        .filter(|(entity_id, _score)| {
                            ...
                            if let Some(loc) = self.location.get(&repr_key) {
                                if loc.state == LocState::Dirty
                                    || loc.state == LocState::Recomputing
                                {
                                    return false;
                                }
                            }
            ...
            Plan::Explain(inner) => {
                ...
                let size = collection_name
                    ...
                    .unwrap_or(0);
                let est =
                    crate::planner::estimate_plan_cost(inner, self.cost_provider.as_ref(), size);
                ...
                rows.push(Row {
                    values: HashMap::from([
                        ...
                        (
                            "value".into(),
                            Value::String(format!("{:.1}", est.total_acu)),
                        ),
                    ]),
                    score: None,
                });
                let breakdown_str = est
                    .items
                    .iter()
                    .map(|item| {
                        format!(
                ...
                if !optimised.rules_applied.is_empty() {
                    rows.push(Row {
                        values: HashMap::from([
                            ...
                            (
                                "value".into(),
                                Value::String(optimised.rules_applied.join(", ")),
                            ),
                        ]),
                        score: None,
                    });
            ...
            Plan::CreateEdgeType(p) => {
                ...
                let decay_config = p
                    .decay_config
                    .as_ref()
                    .map(|dc| crate::edge::DecayConfig {
                        decay_fn: dc.decay_fn.as_ref().map(|f| match f {
                            trondb_tql::DecayFnDecl::Exponential => {
                                crate::edge::DecayFn::Exponential
                            }
                ...
                let payload = rmp_serde::to_vec_named(&edge_type)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::SchemaCreateEdgeType, &p.name, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
            ...
            Plan::InsertEdge(p) => {
                ...
                let tx_id = self.wal.next_tx_id();
                self.wal
                    .append(RecordType::TxBegin, &p.edge_type, tx_id, 1, vec![]);
                let payload = rmp_serde::to_vec_named(&edge)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::EdgeWrite, &p.edge_type, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
            ...
            Plan::DeleteEdge(p) => {
                ...
                .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::EdgeDelete, &p.edge_type, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
                ...
                for _hop in 0..max_depth {
                    ...
                    for node_id in &frontier {
                        ...
                        for entry in &entries {
                            ...
                            if let Ok(entity) =
                                self.store.get(&edge_type.to_collection, &entry.to_id)
                            {
            ...
            Plan::CreateAffinityGroup(_) | Plan::AlterEntityDropAffinity(_) => {
                ...
                Ok(QueryResult {
                    ...
                    rows: vec![Row {
                        values: HashMap::from([(
                            "status".to_owned(),
                            crate::types::Value::String("OK".into()),
                        )]),
                        score: None,
                    }],
                    stats: QueryStats {
            ...
            Plan::DeleteEntity(p) => {
                ...
                .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::EntityDelete, &p.collection, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
                ...
                for (edge_type, to_id) in &forward_edges {
                    self.store
                        .delete_edge(edge_type, entity_id.as_str(), to_id.as_str())?;
                    self.adjacency.remove(&entity_id, edge_type, to_id);
                }
                for (edge_type, from_id) in &backward_edges {
                    self.store
                        .delete_edge(edge_type, from_id.as_str(), entity_id.as_str())?;
                ...
                let _ = self
                    .store
                    .delete_from_tier(&p.collection, &entity_id, Tier::NVMe);
                let _ = self
                    .store
                    .delete_from_tier(&p.collection, &entity_id, Tier::Archive);
            ...
            Plan::ExplainTiers(ref p) => {
                ...
                let hot = self
                    .store
                    .tier_entity_count(&p.collection, Tier::Fjall)
                    .unwrap_or(0);
                let warm = self
                    .store
                    .tier_entity_count(&p.collection, Tier::NVMe)
                    .unwrap_or(0);
                let archive = self
                    .store
                    .tier_entity_count(&p.collection, Tier::Archive)
                    .unwrap_or(0);
            ...
            Plan::Infer(p) => {
                ...
                let source_collection = self
                    ...
                    .ok_or_else(|| {
                        EngineError::InvalidQuery(format!(
                            "entity '{}' not found in any collection",
                            p.from_id
                        ))
                    })?;
                ...
                let source_vector = source_entity
                    .representations
                    .iter()
                    .find_map(|repr| {
                    ...
                    .ok_or_else(|| {
                        EngineError::InvalidQuery(format!(
                            "entity '{}' has no dense vector representation",
                            p.from_id
                        ))
                    })?;
                ...
                let applicable_edge_types: Vec<EdgeType> = if p.edge_types.is_empty() {
                    ...
                    for et_name in &p.edge_types {
                        let et = self.edge_types.get(et_name).ok_or_else(|| {
                            EngineError::InvalidQuery(format!("edge type '{et_name}' not found"))
                        })?;
                ...
                for et in &applicable_edge_types {
                    ...
                    let existing_targets: HashSet<LogicalId> =
                        existing_edges.iter().map(|e| e.to_id.clone()).collect();
                ...
                self.inference_audit.record(InferenceAuditEntry {
                    ...
                    top_candidates: all_candidates
                        .iter()
                        .map(|(_edge_type, to_id, score)| InferenceCandidate {
                ...
                match existing {
                    ...
                    Some(ref _edge) if _edge.source == EdgeSource::Inferred => {
                        ...
                        let payload = rmp_serde::to_vec_named(&new_edge)
                            .map_err(|e| EngineError::Storage(e.to_string()))?;
                        self.wal
                            .append(RecordType::EdgeConfirm, &p.edge_type, tx_id, 1, payload);
                        self.wal.commit(tx_id).await?;
            ...
            Plan::ExplainHistory(p) => {
                ...
                let rows: Vec<Row> = entries
                    ...
                    .map(|e| {
                        ...
                        values.insert(
                            "entity_id".to_string(),
                            Value::String(e.source_entity.to_string()),
                        );
                        values.insert("edge_type".to_string(), Value::String(e.edge_type.clone()));
                        values.insert(
                            "candidates_evaluated".to_string(),
                            Value::Int(e.candidates_evaluated as i64),
                        );
                        values.insert(
                            "candidates_above_threshold".to_string(),
                            Value::Int(e.candidates_above_threshold as i64),
                        );
                        values.insert(
                            "trigger".to_string(),
                            Value::String(format!("{:?}", e.trigger)),
                        );
                        Row {
                            values,
                            score: None,
                ...
                Ok(QueryResult {
                    columns: vec![
                        ...
                        "candidates_above_threshold".into(),
                        "trigger".into(),
                    ],
                    rows,
                    stats: QueryStats {
            ...
            Plan::DropCollection(p) => {
                ...
                let payload = rmp_serde::to_vec_named(&p.name)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::SchemaDropColl, &p.name, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
                ...
                self.sparse_indexes
                    .retain(|k, _| !k.starts_with(&hnsw_prefix));
            ...
            Plan::DropEdgeType(p) => {
                ...
                let payload = rmp_serde::to_vec_named(&p.name)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::SchemaDropEdgeType, &p.name, tx_id, 1, payload);
                self.wal.commit(tx_id).await?;
            ...
            Plan::UpdateEntity(p) => {
                ...
                let changed_fields: HashSet<&str> =
                    p.assignments.iter().map(|(f, _)| f.as_str()).collect();
                ...
                let payload = rmp_serde::to_vec_named(&entity)
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                self.wal
                    .append(RecordType::EntityWrite, &p.collection, tx_id, 1, payload);
                ...
                if let Some(ref s) = schema {
                    for (idx_def, (fidx_key, old_values)) in
                        s.indexes.iter().zip(old_field_values.iter())
                    {
                        if let Some(fidx) = self.field_indexes.get(fidx_key) {
                            // Remove old entry (best-effort — old entry may not exist)
                            let _ = fidx.remove(&entity_id, old_values);
                            // Insert new entry
                            let new_values: Vec<Value> = idx_def
                                .fields
                                .iter()
                                .map(|f| entity.metadata.get(f).cloned().unwrap_or(Value::Null))
                                .collect();
            ...
            Plan::Join(p) => {
                ...
                let mut right_lookup_tables: HashMap<String, HashMap<String, Vec<Entity>>> =
                    HashMap::new();
                for join in &p.joins {
                ...
                let left_entities: Vec<Entity> = self.store.scan(&p.from_collection)?;
                            ...
                            Some(Value::String(ref id_str)) => {
                                if join.confidence_threshold.is_some() {
                                    ...
                                    for et_entry in self.edge_types.iter() {
                                        ...
                                        if et.from_collection
                                            == *alias_to_collection
                                                .get(&join.on_left.alias)
                                            ...
                                            && et.to_collection == join.collection
                                        {
                                            ...
                                            for entry in &entries {
                                                if entry.confidence >= threshold {
                                                    if let Ok(right_ent) = self
                                                        .store
                                                        .get(&join.collection, &entry.to_id)
                                                    {
                                                        if best_match
                                                            .as_ref()
                                                            .map(|(_, c)| entry.confidence > *c)
                                                            .unwrap_or(true)
                                                        {
                                                            best_match =
                                                                Some((right_ent, entry.confidence));
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    if join.on_right.field == "id" {
                                        // Direct ID lookup
                                        self.store
                                            .get(&join.collection, &LogicalId::from_string(id_str))
                                            .ok()
                                            .map(|e| (e, 1.0f32))
                                    } else {
                            ...
                            for right_ent in right_entities {
                                ...
                                let already_matched = rows.iter().any(|r| {
                                    r.values
                                        .get(&right_id_key)
                                        .map(
                                            |v| matches!(v, Value::String(s) if s == &right_id_str),
                                        )
                                        .unwrap_or(false)
                                });
                                if already_matched {
                ...
                Ok(QueryResult {
                    ...
                    rows: vec![Row {
                        values: HashMap::from([
                            ...
                            (
                                "message".into(),
                                Value::String("checkpoint complete".into()),
                            ),
                        ]),
                        score: None,
                    }],
            ...
            Plan::TraverseMatch(p) => self.execute_traverse_match_plan(p).await,
            ...
            Plan::AlterCollection(p) => {
                ...
                match &p.operation {
                    ...
                    trondb_tql::AlterCollectionOp::AddRepresentation { name, .. } => {
                        if schema.representations.iter().any(|r| r.name == *name) {
                            return Err(EngineError::InvalidQuery(format!(
                                "representation '{}' already exists in collection '{}'",
                                name, p.collection
                            )));
                        }
                    }
                ...
                let alter_payload = SchemaAlterPayload {
                    ...
                    op: match &p.operation {
                        ...
                        trondb_tql::AlterCollectionOp::AlterRepresentationSetFields {
                            repr_name,
                            fields,
                        } => SchemaAlterOp::AlterRepresentationSetFields {
                ...
                match &p.operation {
                    trondb_tql::AlterCollectionOp::RenameField { old_name, new_name } => {
                        let field = schema
                            ...
                            .ok_or_else(|| {
                                EngineError::InvalidQuery(format!("field '{old_name}' not found"))
                            })?;
                        field.name = new_name.clone();
                    }
                    trondb_tql::AlterCollectionOp::DropField { field_name } => {
                    ...
                    trondb_tql::AlterCollectionOp::AlterRepresentationSetFields {
                        ...
                        let repr_idx = schema
                            ...
                            .ok_or_else(|| {
                                EngineError::InvalidQuery(format!(
                                    "representation '{repr_name}' not found"
                                ))
                            })?;
                        ...
                        for entity in &scan_entities {
                            ...
                            if self.location.get(&key).is_some() {
                                let _ = self
                                    .location
                                    .transition(&key, crate::location::LocState::Dirty);
                                dirty_count += 1;
                            } else {
                                ...
                                let desc = crate::location::LocationDescriptor {
                                    ...
                                    version: 0,
                                    encoding: if is_sparse {
                                        crate::location::Encoding::Sparse
                                    } else {
                                        crate::location::Encoding::Float32
                                    },
                                    last_accessed: 0,
                                };
                                self.location.register(key, desc);
                    ...
                    trondb_tql::AlterCollectionOp::AlterRepresentationSetModel {
                        ...
                        let vc = repr
                            .vectoriser
                            .get_or_insert(crate::types::VectoriserConfig {
                                model: None,
                                model_path: None,
                                device: None,
                                vectoriser_type: None,
                    ...
                    trondb_tql::AlterCollectionOp::AddRepresentation {
                        ...
                        schema
                            .representations
                            .push(crate::types::StoredRepresentation {
                                name: name.clone(),
                                model: None,
                                dimensions: *dimensions,
                                metric: convert_metric(metric),
                ...
                Ok(QueryResult {
                    ...
                    rows: vec![Row {
                        values: HashMap::from([(
                            "message".into(),
                            Value::String(format!("collection '{}' altered", p.collection)),
                        )]),
                        score: None,
                    }],
                    stats: QueryStats {
            ...
            Plan::Import(p) => {
                ...
                for line in reader.lines() {
                    let line =
                        line.map_err(|e| EngineError::InvalidQuery(format!("read error: {e}")))?;
                    ...
                    let map = obj.as_object().ok_or_else(|| {
                        EngineError::Storage("import: each line must be a JSON object".into())
                    })?;
                    ...
                    self.entity_collections
                        .insert(entity_id, p.collection.clone());
                    imported += 1;
                }
    ...
    pub async fn drain_inference_queue(&self) -> Result<usize, EngineError> {
        let entity_ids: Vec<LogicalId> = self
            .inference_queue
            .iter()
            .map(|r| r.key().clone())
            .collect();
        ...
        for entity_id in &entity_ids {
            ...
            let auto_types: Vec<EdgeType> = self
                .edge_types
                .iter()
                .filter(|et| {
                    et.value().from_collection == collection && et.value().inference_config.auto
                })
            ...
            for et in &auto_types {
                ...
                for (target_id, score) in raw_results {
                    ...
                    let tx_id = self.wal.next_tx_id();
                    self.wal
                        .append(RecordType::TxBegin, &et.name, tx_id, 1, vec![]);
                    let payload = rmp_serde::to_vec_named(&edge)
                        .map_err(|e| EngineError::Storage(e.to_string()))?;
                    self.wal
                        .append(RecordType::EdgeInferred, &et.name, tx_id, 1, payload);
                    self.wal.commit(tx_id).await?;
    ...
    pub async fn sweep_decayed_edges(&self) -> Result<usize, EngineError> {
        ...
        for et_ref in self.edge_types.iter() {
            ...
            for edge in &edges {
                ...
                let effective =
                    crate::edge::effective_confidence(edge.confidence, elapsed, &et.decay_config);
                ...
                if effective < prune_threshold {
                    ...
                    .map_err(|e| EngineError::Storage(e.to_string()))?;
                    self.wal
                        .append(RecordType::EdgeDelete, &et.name, tx_id, 1, payload);
                    self.wal.commit(tx_id).await?;

                    // Fjall delete
                    self.store
                        .delete_edge(&et.name, edge.from_id.as_str(), edge.to_id.as_str())?;
    ...
    pub async fn recompute_dirty(&self) -> Result<usize, EngineError> {
        ...
        if dirty_keys.is_empty() {
            if total_loc > 0 {
                tracing::trace!(
                    total_location_entries = total_loc,
                    "recompute_dirty: no dirty entries"
                );
            }
            return Ok(0);
        }
        tracing::info!(
            dirty = dirty_keys.len(),
            total_loc,
            "recompute_dirty starting batch"
        );

        // Group dirty entries by (collection, repr_index) for batch processing
        let mut groups: std::collections::HashMap<
            (String, usize),
            Vec<(crate::location::ReprKey, crate::types::Entity)>,
        ...
        for ((collection, repr_idx), entries) in &groups {
            ...
            for chunk in entries.chunks(batch_size) {
                ...
                let field_sets: Vec<FieldSet> = chunk
                    .iter()
                    .map(|(_, entity)| {
                        stored_repr
                            .fields
                            ...
                            for source_id in &source_ids {
                                ...
                                for entry in &entries {
                                    if entry.to_id == *node_id {
                                        // Apply temporal filter if present
                                        if let Some(ref tc) = p.temporal {
                                            if !edge_passes_temporal_filter(entry, tc)
                                                .unwrap_or(false)
                                            {
                                                continue;
                                            }
                    ...
                    if include_at_depth {
                        if let Ok(entity) = self.store.get(collection, neighbor_id) {
                            ...
                            if let Some(ref et_name) = edge_type_filter {
                                row.values
                                    .insert("_edge.type".into(), Value::String(et_name.clone()));
                            }
                            row.values
                                .insert("_depth".into(), Value::Int((hop + 1) as i64));
                            rows.push(row);
                            if rows.len() >= limit {
                                break;
                            }
    ...
    async fn execute_brute_force_subgraph(
        ...
        let repr_name = plan.using_repr.as_ref().ok_or_else(|| {
            EngineError::InvalidQuery("WITHIN brute-force requires USING repr_name".into())
        })?;
        ...
        let repr_idx: Option<u32> = self.schemas.get(&plan.collection).and_then(|schema| {
            schema
                .representations
                .iter()
                .position(|r| r.name == *repr_name)
...
fn build_collection_schema(p: &crate::planner::CreateCollectionPlan) -> CollectionSchema {
    let representations: Vec<StoredRepresentation> = p
        .representations
        .iter()
        .map(|r| StoredRepresentation {
            name: r.name.clone(),
            model: r.model.clone(),
    ...
    let vectoriser_config = p
        .vectoriser_config
        .as_ref()
        .map(|vc| crate::types::VectoriserConfig {
...
fn entity_matches(entity: &Entity, clause: &WhereClause) -> bool {
    match clause {
        ...
        WhereClause::IsNull(field) => {
            ...
            !entity.metadata.contains_key(field) || entity.metadata.get(field) == Some(&Value::Null)
        }
...
fn parse_temporal_timestamp(s: &str) -> Result<i64, EngineError> {
    ...
    if let Ok(ms) = s.parse::<i64>() {
        if !(-62167219200000..=253402300799999).contains(&ms) {
            return Err(EngineError::InvalidQuery(format!(
    ...
    if s.len() == 20 && s.ends_with('Z') {
        ...
        if parts.len() == 6 {
            if let (Ok(y), Ok(m), Ok(d), Ok(hh), Ok(mm), Ok(ss)) = (
                ...
                if !(-62167219200000..=253402300799999).contains(&millis_val) {
                    return Err(EngineError::InvalidQuery(format!(
                        "temporal timestamp out of range (years 0-9999): '{s}'"
                    )));
                }
    ...
    Err(EngineError::InvalidQuery(format!(
        "cannot parse temporal timestamp: '{s}'"
    )))
}
...
fn entity_passes_temporal_filter(
    ...
    match clause {
        trondb_tql::TemporalClause::AsOf(ts_str) => {
            ...
            let from_ok = entity.valid_from.is_none_or(|from| from <= point);
            let to_ok = entity.valid_to.is_none_or(|to| point < to);
            Ok(from_ok && to_ok)
        }
...
fn edge_passes_temporal_filter(
    ...
    match clause {
        trondb_tql::TemporalClause::AsOf(ts_str) => {
            let point = parse_temporal_timestamp(ts_str)?;
            let from_ok = edge.valid_from.is_none_or(|from| from <= point);
            let to_ok = edge.valid_to.is_none_or(|to| point < to);
            Ok(from_ok && to_ok)
        }
        trondb_tql::TemporalClause::ValidDuring(start_str, end_str) => {
            let range_start = parse_temporal_timestamp(start_str)?;
...
fn join_row_matches(combined: &HashMap<String, Value>, clause: &WhereClause) -> bool {
    ...
    match clause {
        ...
        WhereClause::IsNull(field) => combined
            .get(field)
            .map(|v| matches!(v, Value::Null))
            .unwrap_or(true),
        WhereClause::IsNotNull(field) => combined
            .get(field)
            .map(|v| !matches!(v, Value::Null))
            .unwrap_or(false),
        WhereClause::In(field, values) => {
        ...
        WhereClause::Like(field, pattern) => combined
            .get(field)
            .and_then(|v| match v {
...
fn build_join_columns(rows: &[Row], fields: &JoinFieldList) -> Vec<String> {
    match fields {
        JoinFieldList::Named(qfs) => qfs
            .iter()
            .map(|qf| format!("{}.{}", qf.alias, qf.field))
            .collect(),
        JoinFieldList::All => {
        ...
        Plan::Search(p) => {
            ...
            if let Some(ref tp) = p.two_pass {
                props.push((
                    ...
                    format!(
                        "first_pass_k={}, binary={}",
                        tp.first_pass_k, tp.use_binary_first_pass
                    ),
                ));
            }
            if !p.hints.is_empty() {
            ...
            if let Some(ref within) = p.within {
                ...
                props.push((
                    ...
                    format!(
                        "TRAVERSE FROM '{}' -[:{}]{} DEPTH {}..{}",
                        within.from_id, edge_type, direction, within.min_depth, within.max_depth,
                    ),
                ));
            }
        }
...
mod tests {
    ...
    use crate::planner::{
        AlterCollectionPlan, BackupPlan, CheckpointPlan, ConfirmEdgePlan, CreateCollectionPlan,
        CreateEdgeTypePlan, DeleteEntityPlan, DropCollectionPlan, DropEdgeTypePlan,
        ExplainHistoryPlan, FetchPlan, FetchStrategy, ImportPlan, InferPlan, InsertEdgePlan,
        InsertPlan, JoinPlan, PreFilter, SearchPlan, TraverseMatchPlan, TraversePlan, UpsertPlan,
    };
    use std::sync::Arc;
    ...
    async fn setup_executor() -> (Executor, tempfile::TempDir) {
        ...
        (
            Executor::with_data_dir(
                store,
                wal,
                Arc::new(LocationTable::new()),
                Arc::new(VectoriserRegistry::new()),
                Arc::new(InferenceAuditBuffer::new(1000)),
    ...
    async fn create_collection_validates_duplicate_repr() {
        ...
        let result = exec
            .execute(&Plan::CreateCollection(CreateCollectionPlan {
                name: "test".into(),
                representations: vec![
    ...
    async fn create_collection_registers_indexes() {
        ...
        assert_eq!(
            exec.indexes().get("docs:dense_repr").unwrap().dimensions(),
            4
        );
        ...
        match err {
            EngineError::InvalidQuery(msg) => {
                assert!(
                    ...
                    "error should mention expected dims: {msg}"
                );
                assert!(
                    msg.contains("got 2"),
                    "error should mention actual dims: {msg}"
                );
            }
    ...
    async fn insert_updates_sparse_index() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "keywords".to_string(),
                VectorLiteral::Sparse(vec![(1, 0.8), (42, 0.5)]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
    ...
    async fn insert_updates_field_index() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "default".to_string(),
                VectorLiteral::Dense(vec![0.1, 0.2, 0.3]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
        ...
        let fidx = exec
            .field_indexes()
            .get("venues:idx_city")
            .expect("field index should exist");
    ...
    async fn fetch_via_field_index() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "default".to_string(),
                VectorLiteral::Dense(vec![0.4, 0.5, 0.6]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
    ...
    async fn fetch_via_field_index_range() {
        ...
        for i in 1..=5 {
            ...
            exec.execute(&Plan::Insert(InsertPlan {
                ...
                values: vec![Literal::String(format!("item{i}")), Literal::Int(score)],
                vectors: vec![(
                    "default".to_string(),
                    VectorLiteral::Dense(vec![0.1, 0.2, 0.3]),
                )],
        ...
        for row in &result.rows {
            ...
            match score {
                Value::Int(v) => assert!(*v >= 30, "score {v} should be >= 30"),
    ...
    async fn fetch_via_field_index_range_and_bounds() {
        ...
        for row in &result.rows {
            let score = row.values.get("score").unwrap();
            match score {
                Value::Int(v) => assert!(*v >= 20 && *v <= 40, "score {v} should be in [20,40]"),
                _ => panic!("expected Int score"),
            }
        }
    }
    ...
    async fn search_sparse_returns_ranked() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "keywords".to_string(),
                VectorLiteral::Sparse(vec![(1, 0.3), (99, 0.9)]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
    ...
    async fn search_hybrid_merges_dense_sparse() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![
                ...
                (
                    "keywords".to_string(),
                    VectorLiteral::Sparse(vec![(1, 0.9)]),
                ),
            ],
            collocate_with: None,
            affinity_group: None,
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![
                ...
                (
                    "keywords".to_string(),
                    VectorLiteral::Sparse(vec![(2, 0.9)]),
                ),
            ],
            collocate_with: None,
            affinity_group: None,
    ...
    async fn search_with_prefilter_narrows_results() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "default".to_string(),
                VectorLiteral::Dense(vec![0.9, 0.1, 0.0]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            fields: vec!["id".into(), "city".into()],
            values: vec![
                Literal::String("v3".into()),
                Literal::String("London".into()),
            ],
            vectors: vec![(
                "default".to_string(),
                VectorLiteral::Dense(vec![0.8, 0.2, 0.0]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
    ...
    async fn explain_shows_fetch_field_index_strategy() {
        ...
        assert_eq!(
            tier_row.values.get("value"),
            Some(&Value::String("FieldIndex".into()))
        );
    }
    ...
    async fn delete_entity_cascading() {
        ...
        assert_eq!(fetch.rows[0].values["id"], Value::String("v2".into()));
    }
    ...
    async fn search_excludes_dirty_entities() {
        ...
        assert_eq!(
            result.rows.len(),
            2,
            "both entities should appear before dirty"
        );
        ...
        location
            .transition(&repr_key, LocState::Recomputing)
            .unwrap();
        // v2: Clean -> Dirty -> Recomputing
        location.transition(&repr_key_v2, LocState::Dirty).unwrap();
        location
            .transition(&repr_key_v2, LocState::Recomputing)
            .unwrap();
        ...
        assert_eq!(
            result.rows.len(),
            0,
            "both recomputing entities should be excluded"
        );
    }
    ...
    impl crate::vectoriser::Vectoriser for TestDenseVectoriser {
        ...
        fn output_kind(&self) -> crate::vectoriser::VectorKind {
            crate::vectoriser::VectorKind::Dense
        }

        async fn encode(
            &self,
            fields: &crate::vectoriser::FieldSet,
        ) -> Result<VectorData, crate::vectoriser::VectoriserError> {
    ...
    async fn wal_replay_repr_write_stores_entity_and_cleans_location() {
        ...
        exec.location
            .transition(&repr_key, LocState::Dirty)
            .unwrap();
        ...
        let fetched_entity = exec
            .store
            .get("events", &LogicalId::from_string("e1"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            exec.entity_collections
                .get(&LogicalId::from_string("e1"))
                .map(|r| r.clone()),
    ...
    async fn wal_replay_repr_write_cleans_recomputing_location() {
        ...
        exec.location
            ...
            .unwrap();
        assert_eq!(
            exec.location.get(&repr_key).unwrap().state,
            LocState::Recomputing
        );
    ...
    async fn wal_replay_repr_dirty_then_repr_write_full_cycle() {
        ...
        let (replayed, unhandled) = exec
            .replay_wal_records(&[dirty_record, write_record])
            .unwrap();
        ...
        let fetched = exec
            .store
            .get("events", &LogicalId::from_string("e3"))
            .unwrap();
        assert_eq!(
    ...
    async fn infer_proposes_edges_sorted_by_similarity() {
        ...
        for row in &result.rows {
            assert_eq!(
                row.values.get("from_id"),
                Some(&Value::String("artist1".into()))
            );
            assert_eq!(
                row.values.get("edge_type"),
                Some(&Value::String("performs_at".into()))
            );
            assert!(row.values.contains_key("to_id"));
            assert!(row.values.contains_key("confidence"));
        }
    ...
    async fn infer_applies_confidence_floor() {
        ...
        insert_entity(&exec, "src", "s1", vec![], Some(vec![1.0, 0.0, 0.0])).await;

        // Insert target with a moderately similar vector
        insert_entity(&exec, "tgt", "t1", vec![], Some(vec![0.5, 0.5, 0.0])).await;
        // Insert target with a very similar vector
        insert_entity(&exec, "tgt", "t2", vec![], Some(vec![0.99, 0.01, 0.0])).await;
    ...
    async fn infer_applies_limit() {
        ...
        assert!(
            result.rows.len() <= 2,
            "INFER should respect the limit: got {} rows",
            result.rows.len()
        );
    }
    ...
    async fn infer_auto_discovers_edge_types() {
        ...
        insert_entity(&exec, "people", "p1", vec![], Some(vec![1.0, 0.0, 0.0])).await;
        insert_entity(&exec, "venues", "v1", vec![], Some(vec![0.9, 0.1, 0.0])).await;
        ...
        assert!(
            ...
            "INFER with empty edge_types should auto-discover applicable types"
        );
        assert_eq!(
    ...
    async fn infer_returns_empty_for_no_applicable_types() {
        ...
        insert_entity(&exec, "lonely", "l1", vec![], Some(vec![1.0, 0.0, 0.0])).await;
        ...
        assert!(
            result.rows.is_empty(),
            "INFER with no applicable edge types should return empty"
        );
    }
    ...
    async fn confirm_creates_new_confirmed_edge() {
        ...
        assert!(
            ...
            "Result message should mention confirmed: {msg}"
        );
    ...
    async fn confirm_rejects_structural_edge() {
        ...
        assert!(
            err_msg.contains("structural"),
            "Error should mention structural: {err_msg}"
        );
    }
    ...
    async fn confirm_updates_existing_confirmed_edge_confidence() {
        ...
        assert!(
            msg.contains("re-confirmed"),
            "Result message should mention re-confirmed: {msg}"
        );
        ...
        assert_eq!(
            ...
            "created_at should be preserved on re-confirmation"
        );
    ...
    async fn explain_history_returns_audit_entries() {
        ...
        let result = exec
            .execute(&Plan::ExplainHistory(ExplainHistoryPlan {
        ...
        assert!(
            result.rows.is_empty(),
            "EXPLAIN HISTORY should return no entries before INFER"
        );

        // Run INFER to generate audit entries
        let infer_result = exec
            .execute(&Plan::Infer(InferPlan {
                from_id: "artist1".into(),
                edge_types: vec!["performs_at".into()],
                limit: Some(10),
            ...
            .unwrap();
        assert!(
            !infer_result.rows.is_empty(),
            "INFER should return proposals"
        );
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "EXPLAIN HISTORY should return 1 audit entry after 1 INFER"
        );

        let row = &result.rows[0];
        assert_eq!(
            row.values.get("entity_id"),
            Some(&Value::String("artist1".into()))
        );
        assert_eq!(
            row.values.get("edge_type"),
        ...
        assert_eq!(
            result.rows.len(),
            2,
            "EXPLAIN HISTORY should return 2 audit entries after 2 INFERs"
        );
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "EXPLAIN HISTORY with limit 1 should return 1 entry"
        );
        ...
        assert!(
            result.rows.is_empty(),
            "EXPLAIN HISTORY for non-inferred entity should be empty"
        );
    }
    ...
    async fn background_inference_creates_inferred_edges() {
        ...
        assert!(
            exec.inference_queue()
                .contains(&LogicalId::from_string("artist1")),
        ...
        assert!(
            ...
            "inference queue should be empty after drain"
        );

        // Verify inferred edges were created
        let edges = exec
            .adjacency()
            .get(&LogicalId::from_string("artist1"), "performs_at");
        ...
        assert!(
            to_ids.contains(&"venue1".to_string()),
            "venue1 should be in inferred edges"
        );
    }
    ...
    async fn inference_queue_populated_on_update() {
        ...
        exec.execute(&Plan::UpdateEntity(crate::planner::UpdateEntityPlan {
            ...
            entity_id: "artist1".into(),
            assignments: vec![("name".into(), Literal::String("Band B".into()))],
        }))
        .await
        .unwrap();
    ...
    async fn drain_inference_queue_skips_existing_edges() {
        ...
        assert_eq!(
            venue1_edges.len(),
            1,
            "should not create duplicate edge to venue1"
        );
        assert_eq!(
            venue1_edges[0].source,
            EdgeSource::Structural,
            "original edge should be Structural"
        );
    }
    ...
    async fn decay_sweeper_prunes_old_inferred_edges() {
        ...
        // Verify inferred edges were created
        let edges_before = exec
            .adjacency()
            .get(&LogicalId::from_string("artist1"), "performs_at");
        assert!(
            !edges_before.is_empty(),
            "inferred edges should have been created"
        );
        ...
        assert!(
            pruned > 0,
            "decay sweeper should have pruned at least one edge"
        );

        // Verify edges are gone from AdjacencyIndex
        let edges_after = exec
            .adjacency()
            .get(&LogicalId::from_string("artist1"), "performs_at");
        assert!(
            edges_after.is_empty(),
            "all inferred edges should have been pruned"
        );

        // Verify edges are gone from Fjall
        let stored_edges = exec.scan_edges("performs_at").unwrap();
        let remaining_inferred: Vec<_> = stored_edges
            .iter()
            .filter(|e| e.source == EdgeSource::Inferred)
            .collect();
        assert!(
            remaining_inferred.is_empty(),
            "no inferred edges should remain in Fjall"
        );
    }
    ...
    async fn decay_sweeper_skips_structural_and_confirmed_edges() {
        ...
        let edges_before = exec
            .adjacency()
            .get(&LogicalId::from_string("alice"), "knows");
        ...
        assert_eq!(
            edges_after.len(),
            1,
            "structural edge should remain after sweep"
        );
    }
    ...
    async fn fetch_where_neq() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("status", Literal::String("active".into()))],
            None,
        )
        .await;
        insert_entity(
            ...
            "venues",
            "v2",
            vec![("status", Literal::String("archived".into()))],
            None,
        )
        .await;
        insert_entity(
        ...
        let result = exec
            .execute(&Plan::Fetch(FetchPlan {
                collection: "venues".into(),
                fields: FieldList::All,
                filter: Some(WhereClause::Neq(
                    "status".into(),
                    Literal::String("archived".into()),
                )),
                temporal: None,
                order_by: vec![],
                limit: None,
    ...
    async fn fetch_where_is_null() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("name", Literal::String("Blue Note".into()))],
            None,
        )
        .await;
    ...
    async fn fetch_where_in() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("category", Literal::String("music".into()))],
            None,
        )
        .await;
        insert_entity(
            ...
            "venues",
            "v2",
            vec![("category", Literal::String("theatre".into()))],
            None,
        )
        .await;
        insert_entity(
            ...
            "venues",
            "v3",
            vec![("category", Literal::String("food".into()))],
            None,
        )
        .await;
    ...
    async fn fetch_where_like() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("name", Literal::String("Jazz Cafe".into()))],
            None,
        )
        .await;
        insert_entity(
            ...
            "venues",
            "v2",
            vec![("name", Literal::String("Jazz Club Bristol".into()))],
            None,
        )
        .await;
        insert_entity(
    ...
    async fn fetch_where_not() {
        ...
        let result = exec
            .execute(&Plan::Fetch(FetchPlan {
                collection: "venues".into(),
                fields: FieldList::All,
                filter: Some(WhereClause::Not(Box::new(WhereClause::Eq(
                    "active".into(),
                    Literal::Bool(true),
                )))),
                temporal: None,
    ...
    async fn fetch_order_by_asc() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("name", Literal::String("Charlie".into()))],
            None,
        )
        .await;
        insert_entity(
        ...
        insert_entity(
            ...
            "venues",
            "v3",
            vec![("name", Literal::String("Bravo".into()))],
            None,
        )
        .await;
        ...
        assert_eq!(
            result.rows[1].values.get("name").unwrap(),
            &Value::String("Bravo".into())
        );
        assert_eq!(
            result.rows[2].values.get("name").unwrap(),
    ...
    async fn join_inner_structural() {
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            values: vec![
                Literal::String("v1".into()),
                Literal::String("123 Main St".into()),
            ],
            vectors: vec![(
                "default".into(),
                trondb_tql::VectorLiteral::Dense(vec![1.0, 0.0, 0.0]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
        ...
        exec.execute(&Plan::Insert(InsertPlan {
            ...
            vectors: vec![(
                "default".into(),
                trondb_tql::VectorLiteral::Dense(vec![0.0, 1.0, 0.0]),
            )],
            collocate_with: None,
            affinity_group: None,
            valid_from: None,
        ...
        let result = exec
            .execute(&Plan::Join(JoinPlan {
                ...
                joins: vec![JoinClause {
                    ...
                    collection: "venues".into(),
                    alias: "v".into(),
                    on_left: QualifiedField {
                        alias: "p".into(),
                        field: "venue_id".into(),
                    },
                    on_right: QualifiedField {
                        alias: "v".into(),
                        field: "id".into(),
                    },
    ...
    async fn traverse_match_forward() {
        ...
        let result = exec
            .execute(&Plan::TraverseMatch(TraverseMatchPlan {
                from_id: "a".into(),
    ...
    async fn traverse_match_edge_type_metadata() {
        ...
        assert_eq!(result.rows[0].values.get("_depth"), Some(&Value::Int(1)));
        assert_eq!(
    ...
    async fn e2e_join_and_traverse_match() {
        ...
        insert_entity_full(
            ...
            "v1",
            vec![
                ("address", Literal::String("100 Main St".into())),
                ("capacity", Literal::Int(500)),
            ],
            None,
            None,
        )
        ...
        insert_entity_full(
            ...
            "v2",
            vec![
                ("address", Literal::String("200 Oak Ave".into())),
                ("capacity", Literal::Int(1000)),
            ],
            None,
            None,
        )
        ...
        insert_entity_full(
            ...
            "a1",
            vec![
                ("name", Literal::String("Alice".into())),
                ("genre", Literal::String("jazz".into())),
                ("venue_id", Literal::String("v1".into())),
            ],
            None,
            None,
        )
        ...
        insert_entity_full(
            ...
            "a2",
            vec![
                ("name", Literal::String("Bob".into())),
                ("genre", Literal::String("rock".into())),
                ("venue_id", Literal::String("v2".into())),
            ],
            None,
            None,
        )
        ...
        insert_entity_full(
            ...
            "a3",
            vec![
                ("name", Literal::String("Charlie".into())),
                ("genre", Literal::String("blues".into())),
                ("venue_id", Literal::String("nonexistent".into())),
            ],
            None,
            None,
        )
        ...
        // ---------------------------------------------------------------
        {
            ...
            let addresses: Vec<&Value> = result
                .rows
                .iter()
        ...
        {
            let stmt =
                trondb_tql::parse("TRAVERSE FROM 'a1' MATCH (x)-[e:collaborates]->(y) DEPTH 1..2;")
                    .unwrap();
            let p = crate::planner::plan(&stmt, &exec.schemas).unwrap();
        ...
        // ---------------------------------------------------------------
        {
            let stmt = trondb_tql::parse("TRAVERSE collaborates FROM 'a1' DEPTH 2;").unwrap();
            let p = crate::planner::plan(&stmt, &exec.schemas).unwrap();
    ...
    async fn e2e_inner_join_with_where() {
        ...
        insert_entity_full(
            ...
            "venues",
            "v1",
            vec![("address", Literal::String("123 Main St".into()))],
            None,
            None,
        )
        .await;
        insert_entity_full(
            ...
            "venues",
            "v2",
            vec![("address", Literal::String("456 Oak Ave".into()))],
            None,
            None,
        )
        .await;
    ...
    async fn e2e_traverse_match_with_limit() {
        ...
        for (id, label) in &[("n1", "Start"), ("n2", "Mid"), ("n3", "End")] {
            insert_entity_full(
                ...
                "nodes",
                id,
                vec![("label", Literal::String(label.to_string()))],
                None,
                None,
            )
            .await;
        ...
        let stmt =
            trondb_tql::parse("TRAVERSE FROM 'n1' MATCH (x)-[e:links]->(y) DEPTH 1..2 LIMIT 1;")
                .unwrap();
        let p = crate::planner::plan(&stmt, &exec.schemas).unwrap();
        let result = exec.execute(&p).await.unwrap();
    ...
    async fn e2e_join_wildcard_select() {
        ...
        insert_entity_full(
            ...
            "categories",
            "c1",
            vec![("label", Literal::String("Books".into()))],
            None,
            None,
        )
        .await;
        insert_entity_full(
            ...
            "i1",
            vec![
                ("title", Literal::String("Rust in Action".into())),
                ("cat_id", Literal::String("c1".into())),
            ],
            None,
            None,
        )
        ...
        assert_eq!(
            row.values.get("i.title"),
            Some(&Value::String("Rust in Action".into()))
        );
        assert_eq!(
            row.values.get("c.label"),
    ...
    async fn explain_shows_cost_breakdown() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("name", Literal::String("The Fleece".into()))],
            Some(vec![1.0, 0.0, 0.0]),
        )
        .await;
        insert_entity(
            ...
            "venues",
            "v2",
            vec![("name", Literal::String("The Exchange".into()))],
            Some(vec![0.0, 1.0, 0.0]),
        )
        .await;
    ...
    async fn query_result_includes_cost() {
        ...
        assert!(
            result.stats.cost.is_some(),
            "QueryStats should include cost estimate"
        );
    ...
    async fn warnings_propagated_to_result() {
        ...
        let explain_result = exec.execute(&Plan::Explain(Box::new(plan))).await.unwrap();
        ...
        assert!(
            has_warnings,
            "EXPLAIN should show warnings for deep TRAVERSE"
        );
    }
    ...
    fn test_entity_temporal_valid_during_overlap() {
        ...
        let clause =
            trondb_tql::TemporalClause::ValidDuring("2024-06-01".into(), "2024-12-01".into());
        assert!(entity_passes_temporal_filter(&entity, &clause).unwrap());
    }
    ...
    fn test_entity_temporal_valid_during_no_overlap() {
        ...
        let clause =
            trondb_tql::TemporalClause::ValidDuring("2025-01-01".into(), "2025-06-01".into());
        assert!(!entity_passes_temporal_filter(&entity, &clause).unwrap());
    }
    ...
    async fn execute_fetch_as_of_filters_entities() {
        ...
        let result = exec
            .execute(&Plan::Fetch(FetchPlan {
                ...
                fields: FieldList::All,
                filter: None,
                temporal: Some(trondb_tql::TemporalClause::AsOf(
                    "2024-08-01T00:00:00Z".into(),
                )),
                order_by: vec![],
                limit: None,
                strategy: FetchStrategy::FullScan,
        ...
        assert_eq!(
            result.rows[0].values.get("name"),
            Some(&Value::String("Current Event".into()))
        );
    }
    ...
    async fn execute_fetch_valid_during() {
        ...
        let result = exec
            .execute(&Plan::Fetch(FetchPlan {
                ...
                fields: FieldList::All,
                filter: None,
                temporal: Some(trondb_tql::TemporalClause::ValidDuring(
                    "2024-03-01".into(),
                    "2024-09-01".into(),
                )),
                order_by: vec![],
    ...
    async fn execute_fetch_as_of_transaction() {
        ...
        insert_entity(
            ...
            "events",
            "e1",
            vec![("name", Literal::String("Early".into()))],
            None,
        )
        .await;
        ...
        insert_entity(
            ...
            "events",
            "e2",
            vec![("name", Literal::String("Late".into()))],
            None,
        )
        .await;
        ...
        let all = exec
            .execute(&Plan::Fetch(FetchPlan {
                collection: "events".into(),
                fields: FieldList::All,
                filter: Some(trondb_tql::WhereClause::Eq(
                    "id".into(),
                    trondb_tql::Literal::String("e1".into()),
                )),
                temporal: None,
                order_by: vec![],
                limit: None,
    ...
    async fn e2e_explain_temporal() {
        ...
        let result = exec
            .execute(&Plan::Explain(Box::new(Plan::Fetch(FetchPlan {
                collection: "events".into(),
                fields: FieldList::All,
                filter: None,
                temporal: Some(trondb_tql::TemporalClause::ValidDuring(
        ...
        let temporal_shown = result
            .rows
            .iter()
            .any(|r| r.values.get("property") == Some(&Value::String("temporal".into())));
    ...
    async fn execute_upsert_update_when_exists() {
        ...
        insert_entity(
            ...
            "venues",
            "v1",
            vec![("name", Literal::String("Old".into()))],
            None,
        )
        .await;
    ...
    async fn execute_checkpoint() {
        ...
        let result = exec
            .execute(&Plan::Checkpoint(CheckpointPlan))
            .await
            .unwrap();
        assert_eq!(result.rows.len(), 1);
    ...
    async fn execute_backup_creates_files() {
        ...
        let result = exec
            .execute(&Plan::Backup(BackupPlan {
    ...
    async fn execute_restore_returns_message() {
        ...
        let result = exec
            .execute(&Plan::Restore(crate::planner::RestorePlan {
        ...
        assert_eq!(result.rows.len(), 1);
        assert!(result.rows[0].values.contains_key("message"));
    }
    ...
    async fn execute_alter_collection_rename_field() {
        ...
        insert_entity(
            ...
            "test_alter",
            "e1",
            vec![("old_name", Literal::String("value".into()))],
            None,
        )
        .await;
...
"#,
        ...
        let result = exec
            .execute(&Plan::Import(ImportPlan {
                collection: "venues".into(),
        ...
        assert_eq!(result.rows.len(), 1);
        assert_eq!(result.rows[0].values.get("imported"), Some(&Value::Int(2)));
    ...
    async fn integration_phase15_upsert_checkpoint_alter_import() {
        ...
        assert!(
            !after_rename.rows[0].values.contains_key("city"),
        ...
        std::fs::write(
            &import_path,
            r#"{"id":"e2","name":"Imported1","location":"Berlin"}
...
"#,
        ...
        let final_fetch = exec
            ...
            .unwrap();
        assert_eq!(
            final_fetch.rows.len(),
            3,
            "should have 3 entities total after import"
        );

        // --- Metrics verification ---
        assert!(
            exec.engine_metrics().queries_total.get() > 0,
            "queries_total should be incremented"
        );
        assert!(
            exec.engine_metrics().inserts_total.get() > 0,
            "inserts_total should be incremented"
        );
    }
    ...
    async fn alter_collection_schema_alter_wal_record() {
        ...
        insert_entity(
            ...
            "wal_test",
            "e1",
            vec![("old_field", Literal::String("val".into()))],
            None,
        )
        .await;
        ...
        assert!(
            schema.fields.iter().any(|f| f.name == "new_field"),
            "schema should have new_field"
        );
        assert!(
            !schema.fields.iter().any(|f| f.name == "old_field"),
            "schema should not have old_field"
        );
        ...
        assert_eq!(result.rows.len(), 1);
        assert!(
            result.rows[0].values.contains_key("new_field"),
            "entity should have new_field"
        );
        assert!(
            !result.rows[0].values.contains_key("old_field"),
            "entity should not have old_field"
        );
    }
    ...
    async fn alter_collection_wal_replay_rename() {
        ...
        assert!(
            schema.fields.iter().any(|f| f.name == "new_field"),
            "replayed schema should have new_field"
        );
        assert!(
            !schema.fields.iter().any(|f| f.name == "old_field"),
            "replayed schema should not have old_field"
        );
    }
    ...
    async fn alter_collection_wal_replay_drop() {
        ...
        assert!(
            schema.fields.iter().any(|f| f.name == "keep_me"),
            "keep_me should remain"
        );
        assert!(
            !schema.fields.iter().any(|f| f.name == "drop_me"),
            "drop_me should be removed by replay"
        );
    }
    ...
    async fn search_within_traverse_brute_force() {
        ...
        for (id, vec) in &[
            ...
            exec.execute(&Plan::Insert(InsertPlan {
                ...
                vectors: vec![(
                    "dense".into(),
                    trondb_tql::VectorLiteral::Dense(vec.clone()),
                )],
                collocate_with: None,
                affinity_group: None,
                valid_from: None,
        ...
        assert_eq!(
            ids[0], "b",
            "b should rank first (highest cosine similarity to [1,0,0])"
        );
    }
    ...
    async fn search_within_empty_subgraph() {
        ...
        assert_eq!(
            result.rows.len(),
            0,
            "expected 0 results for empty subgraph"
        );
    }
    ...
    async fn search_within_traverse_end_to_end() {
        ...
        for (id, title, vec) in &[
            ...
            exec.execute(&Plan::Insert(InsertPlan {
                ...
                values: vec![
                    Literal::String(id.to_string()),
                    Literal::String(title.to_string()),
                ],
                vectors: vec![(
                    "dense".into(),
                    trondb_tql::VectorLiteral::Dense(vec.clone()),
        ...
        assert_eq!(
            ids.len(),
            2,
            "expected 2 results (spoke1 and spoke2), got: {ids:?}"
        );
        assert!(
            ids.contains(&"spoke1".to_string()),
            "expected spoke1 in results"
        );
        assert!(
            ids.contains(&"spoke2".to_string()),
            "expected spoke2 in results"
        );
        assert!(
            !ids.contains(&"island".to_string()),
            "island must not appear — it is disconnected"
        );
        // spoke1 should rank first (closest to query vector [0.9, 0.1, 0.0])
        assert_eq!(
            ids[0], "spoke1",
            "spoke1 should rank first (highest cosine similarity to query)"
        );
    }
    ...
    async fn explain_search_within() {
        ...
        let within_row = result
            .rows
            .iter()
            .find(|r| r.values.get("property") == Some(&Value::String("within".into())));
        assert!(
            within_row.is_some(),
            "EXPLAIN should emit a 'within' property row"
        );
