...
impl AdjacencyIndex {
    ...
    // Nine parameters because an edge write carries its full identity, decay
    // configuration, and provenance in one call; splitting it into a builder
    // would hide which fields are required.
    ...
    pub fn insert(
        ...
        let mut entries = self.forward.entry(key).or_default();
        ...
        let mut back_entries = self.backward.entry(bkey).or_default();
