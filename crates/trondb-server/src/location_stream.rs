            ...
            loop {
                match rx.recv().await {
                    ...
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        ...
                        eprintln!("[location_stream] receiver lagged by {n} messages");
                    }
...
mod tests {
    ...
    async fn location_stream_sends_snapshot_then_deltas() {
        ...
        let _handle = spawn_location_stream(engine.clone(), Some(bcast_rx), tx);
        ...
        assert!(
            ...
            "snapshot payload should not be empty"
        );
