...
impl TronNodeService {
    ...
    fn engine_ref(&self) -> Result<&Arc<Engine>, Status> {
        self.engine
            .as_ref()
            .ok_or_else(|| Status::failed_precondition("no local engine (router node)"))
    }
}
...
impl TronNode for TronNodeService {
    async fn execute(
        ...
        let plan = Plan::try_from(proto_plan).map_err(Status::invalid_argument)?;
        ...
        if self.role == NodeRoleConfig::Primary && Self::is_write_plan(&plan) {
            if let Some(tracker) = &self.tracker {
                ...
                if lsn_after > lsn_before && tracker.connected_count() > 0 {
                    ...
                    if let Ok(records) = engine.wal_records_since(lsn_before).await {
                        ...
                        if let Some(ref loc_tx) = self.location_broadcast {
                            for record in &records {
                                if matches!(
                                    record.record_type,
                                    trondb_wal::RecordType::LocationUpdate
                                ) {
                    ...
                    match tracker
                        .wait_for_ack(lsn_after, cfg.min_ack_replicas, timeout)
                        .await
                    {
    ...
    async fn stream_health(
        ...
        spawn_health_stream(engine.clone(), self.role, tx, Duration::from_millis(200));
    ...
    async fn stream_wal(
        ...
        tokio::spawn(async move {
            while let Some(result) = client_stream.next().await {
                match result {
                    Ok(ack) => {
                        tracker_clone.update_confirmed_lsn(&replica_id_clone, ack.confirmed_lsn);
                    }
...
mod tests {
    ...
    async fn wal_stream_catches_up_replica() {
        ...
        assert!(
            head_lsn > 0,
            "expected WAL records after insert, head_lsn={head_lsn}"
        );
    ...
    async fn semi_sync_write_waits_for_replica_ack() {
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "expected status row from CreateCollection"
        );

        // The write should have waited for the replica ack (~50ms delay)
        assert!(
            elapsed >= Duration::from_millis(40),
            "expected execute to wait for replica ack (>=40ms), but it took only {elapsed:?}"
        );
    }
    ...
    async fn semi_sync_require_replica_fails_when_no_replicas() {
        ...
        assert!(
            result.is_err(),
            "expected error when require_replica=true and no replicas"
        );
        let status = result.unwrap_err();
    ...
    async fn semi_sync_no_tracker_succeeds_immediately() {
        ...
        assert!(
            ...
            "expected fast execution without tracker, took {elapsed:?}"
        );
    }
}
