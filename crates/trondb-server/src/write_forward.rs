...
mod tests {
    ...
    async fn forward_create_collection_from_replica_to_primary() {
        ...
        let primary_service = TronNodeService::new(primary_engine.clone(), NodeRoleConfig::Primary);
        ...
        assert_eq!(
            result.rows.len(),
            1,
            "expected one status row from CreateCollection"
        );
    ...
    async fn forward_from_router_to_primary() {
        ...
        assert!(
            primary_engine
                .collections()
                .contains(&"router_forwarded".to_string()),
    ...
    async fn forward_without_primary_channel_returns_unavailable() {
        ...
        assert!(
            result.is_err(),
            "expected error when no primary channel is configured"
        );
